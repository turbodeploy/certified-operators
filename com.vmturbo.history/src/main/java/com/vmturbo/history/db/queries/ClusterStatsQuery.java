...
public class ClusterStatsQuery extends QueryBase {
    ...
    public ClusterStatsQuery(@Nonnull Table<?> table,
        ...
        super(dsl);
        addTable(table);
        final Field<Timestamp> recordedOn = JooqUtils.getTimestampField(table,
                StringConstants.RECORDED_ON);
        final Field<String> propertyType = JooqUtils.getStringField(table,
                StringConstants.PROPERTY_TYPE);
