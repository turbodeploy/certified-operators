 ...
 */
public class CommodityCache implements MemReporter {
     ...
     * @param oidPack                data pack so we can represent oids as ints in data structures
     */
    public CommodityCache(Set<CommodityType> excludedCommodityTypes, IDataPack<Long> oidPack) {
    ...
    public void cacheUsagesAndCapacity(TopologyDTO.TopologyEntityDTO entity) {
        final Iterator<CommoditySoldDTO> soldCommodities =
                ...
                if (sold.hasHistoricalUsed()) {
                    ...
                    if (historicalValues.hasMovingMeanPlusStandardDeviations()) {
                        entitySmoothedUsageMap.put(index,
                                (float)(historicalValues.getMovingMeanPlusStandardDeviations()
                                        / sold.getCapacity()));
                        ...
                        entitySmoothedUsageMap.put(index,
                                (float)(historicalValues.getHistUtilization()
                                        / sold.getCapacity()));
                    }
                }
            });
     ...
     */
    public Double getCapacity(final long providerId,
     ...
     * {@link #getCapacity(long, TopologyDTO.CommodityType)} is recommended, since it does not carry
     * the overhead of constructing {@link CommodityType} instaces.</p>
     *
     ...
     * CommodityType} to smoothed usage value.
     *
    ...
    public Map<Integer, Double> getEntityUsageStats(final long entityId) {
        ...
        smoothedUsage.getOrDefault(oidPack.toIndex(entityId), Int2FloatMaps.EMPTY_MAP).forEach(
                (index, usage) ->
                        result.put(commodityTypePack.fromIndex(index).getType(),
                                Double.valueOf(usage)));
        return result;
    }
    ...
    public Map<Integer, Double> getEntityCapacityStats(final long entityId) {
        Map<Integer, Double> result = new HashMap<>();
        capacities.getOrDefault(oidPack.toIndex(entityId), Int2FloatMaps.EMPTY_MAP).forEach(
                (index, capacity) ->
                        result.put(commodityTypePack.fromIndex(index).getType(),
