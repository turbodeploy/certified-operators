...
public class LiveActionStore implements ActionStore {
    ...
    public boolean populateRecommendedActions(@Nonnull final ActionPlan actionPlan)
        ...
        synchronized (storePopulationLock) {
            ...
            populateAtomicActions(planId, aggregatedActions, mergedActionViews.values(),
            ...
            final int deletedActions =
                entitiesWithNewStateCache.clearActionsAndUpdateCache(topologyId);
     ...
     * @param mergedActions map containing the Action Id of the market action that was merged
     ...
     * @param translatedActionsToAdd used as a return value to track the translated actions that
     *                               come from atomic actions.
     * @return true if the atomic actions are updated without any errors
     */
    public boolean populateAtomicActions(long planId,
                                          Map<Long, AggregatedAction> aggregatedActions,
                                          Collection<Action> mergedActions,
                                          RecommendationTracker lastExecutedRecommendationsTracker,
                                          EntitiesAndSettingsSnapshot snapshot,
                                          List<Action> translatedActionsToAdd) {
