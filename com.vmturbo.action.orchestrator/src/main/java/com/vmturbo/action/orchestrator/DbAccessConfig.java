 ...
 */
@Configuration
@Import({ActionOrchestratorDBConfig.class, ActionOrchestratorDbEndpointConfig.class})
public class DbAccessConfig {

    @Autowired(required = false)
    private ActionOrchestratorDBConfig actionOrchestratorDBConfig;

    @Autowired(required = false)
    private ActionOrchestratorDbEndpointConfig actionOrchestratorDBEndpointConfig;
    ...
    public void startDbMonitor() {
        if (dbMonitorEnabled) {
            if (actionOrchestratorDBConfig != null) {
                actionOrchestratorDBConfig.startDbMonitor();
            } else {
                actionOrchestratorDBEndpointConfig.actionOrchestratorEndpoint().startDbMonitor();
            }
        }
    }
    ...
    public DSLContext dsl() throws SQLException, UnsupportedDialectException, InterruptedException {
        return FeatureFlags.POSTGRES_PRIMARY_DB.isEnabled()
                ? actionOrchestratorDBEndpointConfig.actionOrchestratorEndpoint().dslContext()
                : actionOrchestratorDBConfig.dsl();
    }
    ...
    public DataSource dataSource() throws SQLException, UnsupportedDialectException, InterruptedException {
        return FeatureFlags.POSTGRES_PRIMARY_DB.isEnabled()
                ? actionOrchestratorDBEndpointConfig.actionOrchestratorEndpoint().datasource()
                : actionOrchestratorDBConfig.dataSource();
    }
}
