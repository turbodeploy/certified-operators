...
public class ActionTargetSelector {
     ...
     * one action, use {@link ActionTargetSelector#getTargetsForActions(Stream, EntitiesAndSettingsSnapshot, Map)}.
     *
     ...
     * @param workflowExecutionTargetId id of the execution target for action with REPLACE workflow
     * @return An {@link ActionTargetInfo} describing the support + target id for the action.
     */
    @Nonnull
    public ActionTargetInfo getTargetForAction(@Nonnull final ActionDTO.Action action,
            @Nonnull final EntitiesAndSettingsSnapshotFactory entitySettingsCache,
            @Nonnull final Optional<Long> workflowExecutionTargetId) {
        final Map<Long, Long> workflowExecutionTargetForAction = new HashMap<>();
        workflowExecutionTargetId.ifPresent(
                targetId -> workflowExecutionTargetForAction.put(action.getId(), targetId));
        return getTargetsForActions(Stream.of(action), entitySettingsCache.emptySnapshot(),
                workflowExecutionTargetForAction).get(action.getId());
    }

    /**
     * Returns an {@link ActionTargetInfo} for each provided action.
     * This is the "bulk" version of
     * {@link ActionTargetSelector#getTargetForAction(Action, EntitiesAndSettingsSnapshotFactory, Optional)}.
     *
    ...
    public Map<Long, ActionTargetInfo> getTargetsForActions(
            ...
            @Nonnull final Map<Long, Long> workflowExecutionTargetsForActions) {
        ...
        try {
            actionList.forEach(action -> targetsForActions.put(action.getId(),
                    targetInfoResolver.getTargetInfoForAction(action, actionPartialEntityMap,
                            snapshot, workflowExecutionTargetsForActions)));
