...
public abstract class DbAdapter {
     ...
     *
     * <p>Note: A single instance of the pool will be shared from {@link PooledDataSourceSingleton}</p>
     *
    ...
    DataSource getDataSource(boolean pooled) throws UnsupportedDialectException, SQLException {
        return pooled ? PooledDataSourceSingleton.getInstance(this) : createDataSource(false);
    }

    /**
     * Create a new {@link DataSource} using the {@link DbEndpointConfig} url, username, and password.
     ...
     * @return datasource
     * @throws SQLException if the datasource cannot be created
     * @throws UnsupportedDialectException if dialect is not supported
     */
    protected DataSource createDataSource(boolean pooled)
