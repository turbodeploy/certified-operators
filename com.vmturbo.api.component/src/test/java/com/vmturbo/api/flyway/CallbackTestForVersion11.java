...
/**
 * Flyway callback test class for fixing the V1_1 migration.
 */
public class CallbackTestForVersion11 extends Assert {
    ...
    public void testNoUpdateWhenNoVersionTable() throws SQLException {
        ...
        when(statement.executeQuery(MigrationCallbackForVersion11.GET_DATABASE_QUERY)).thenReturn(
                dbResults);

        when(dbResults.getString(1)).thenReturn(TEST_DB);

        final ResultSet results = mock(ResultSet.class);
        when(statement.executeQuery(TABLE_EXISTS_QUERY)).thenReturn(results);
        when(results.next()).thenReturn(false);

        final MigrationCallbackForVersion11 callback = new MigrationCallbackForVersion11();
    ...
    /**
     * Test scenario when there's a version table but no record for the V1_1 migration.
     *
    ...
    public void testNoUpdateWhenVersionNotFound() throws SQLException {
        ...
        when(statement.executeQuery(TABLE_EXISTS_QUERY)).thenReturn(tableResults);
        ...
        when(statement.executeQuery(MigrationCallbackForVersion11.GET_CHECKSUM_QUERY)).thenReturn(
                migrationResults);
     ...
     * correct checksum.
     *
     * <p>This can happen either after an upgrade from a version that had V1_1 to one with the
     * correct V1_1 migration, or after the incorrect V1_1 checksum has been fixed during
     * a prior launch.</p>
     *
     * @throws SQLException must be declared but will not be thrown.
     */
    ...
    public void testNoUpdateWhenCorrectMigrationChecksum() throws SQLException {
        ...
        when(migrationResults.getInt(1)).thenReturn(
                MigrationCallbackForVersion11.V1_1_CORRECT_CHECKSUM);
    ...
    /**
     * Scenario when the version table exists and contains a V1_1 migration record with an
     * incorrect checksum.
     *
     * <p>This will happen after an upgrade from a version that contains the incorrect V1_1
     * migration.</p>
     *
    ...
    public void testUpdateWhenIncorrectMigrationChecksum() throws SQLException {
        ...
        when(migrationResults.getInt(1)).thenReturn(
                MigrationCallbackForVersion11.V1_1_CORRECT_CHECKSUM - 1);
    ...
    private void verifyStatementCounts(int tblExistsCount, int getChecksumCount, int updateCount)
        ...
        verify(statement, times(getChecksumCount)).executeQuery(
                MigrationCallbackForVersion11.GET_CHECKSUM_QUERY);
        verify(statement, times(updateCount)).executeUpdate(
                MigrationCallbackForVersion11.UPDATE_CHECKSUM_STMT);
    }
}
