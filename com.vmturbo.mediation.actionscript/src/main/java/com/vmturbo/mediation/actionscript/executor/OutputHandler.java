...
class OutputHandler extends Observable implements AutoCloseable {
     ...
     */
    OutputHandler(final @Nonnull PipedOutputStream out, final int maxOutputLines) {
    ...
    void finish(final @Nonnull OutputStream otherPipeEnd) {
        try {
            // The write end of the pipe should already be closed at this point, since this method
            // is called after the SSH client-side event handler has been told that the remote
            // execution has completed. But just in case, we'll attempt to close it here, so if
            // it somehow wasn't already closed by the SSH channel, we'll have a chance to capture
            // any still-buffered output and won't leak an open stream
            otherPipeEnd.close();
            logger.warn("STDOUT was unexpectedly not closed when finalizing an SSH output handler");
        } catch (IOException e) {
            // This is expected, so no logging here
        }
