...
public class SystemSshdLauncher {
    private static Logger log = LogManager.getLogger(SystemSshdLauncher.class);
    ...
    public int launch() throws IOException {
        ...
        setPerms();
        log.info("Running sshd on port {} in dir {}", port, tempDir);
    ...
    protected void finalize() throws Throwable {
        if (process != null) {
            log.warn("SystemSshdLauncher was not closed");
            close();
        }
    }
    ...
    private Process buildProcess(File configFile) throws IOException {
        ...
        do {
            try {
                Thread.currentThread().sleep(1000L); // give the sshd little time to start before connections are requested
                interrupted = false;
