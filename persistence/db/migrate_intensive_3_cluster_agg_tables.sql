...
/*
 * RECREATE THE DATA THAT MIGHT BE MISSING DUE TO THE MISSING PROCEDURES 
 */
insert into vm_stats_by_month select * from vm_summary_stats_by_month 
where 
snapshot_time > IFNULL((select max(snapshot_time) as t1 from vm_stats_by_month where property_type = 'priceIndex'), 0)
and snapshot_time<start_of_month_ms(date(now()));

insert into pm_stats_by_month select * from pm_summary_stats_by_month 
where 
snapshot_time > IFNULL((select max(snapshot_time) as t1 from pm_stats_by_month where property_type = 'priceIndex'), 0)
and snapshot_time<start_of_month_ms(date(now()));

insert into ds_stats_by_month select * from ds_summary_stats_by_month 
where 
snapshot_time > IFNULL((select max(snapshot_time) as t1 from ds_stats_by_month where property_type = 'priceIndex'), 0)
and snapshot_time<start_of_month_ms(date(now()));

insert into vdc_stats_by_month select * from vdc_summary_stats_by_month 
where 
snapshot_time > IFNULL((select max(snapshot_time) as t1 from vdc_stats_by_month where property_type = 'priceIndex'), 0)
and snapshot_time<start_of_month_ms(date(now()));

insert into app_stats_by_month select * from app_summary_stats_by_month 
where 
snapshot_time > IFNULL((select max(snapshot_time) as t1 from app_stats_by_month where property_type = 'priceIndex'), 0)
and snapshot_time<start_of_month_ms(date(now()));
...
/* DAILY */
delimiter //
drop procedure if exists migrate_clusterAgg_Insert_SinceLastDays//
create procedure migrate_clusterAgg_Insert_SinceLastDays(IN cluster_internal_name varchar(250))
begin

/* Default to populate all of the available previous days */
set @last_stats = IFNULL((select max(recorded_on) from cluster_stats_by_day 
	where convert(internal_name using utf8) = convert(cluster_internal_name using utf8)), 
(select date(date_sub(now(), interval 60 day)) as d));
...
from
	...
	natural join
		...
		where 
			convert(internal_name using utf8) = replace(concat('VMs_',convert(cluster_internal_name using utf8)),'VMs_GROUP-PMs','GROUP-VMs')
			and date(recorded_on) > @last_stats
		) as members
	group by 
		recorded_on,
		uuid
...
delimiter $$
drop procedure if exists migrate_populateAll_DailyClusterAggTable_SinceLastDays$$
create procedure migrate_populateAll_DailyClusterAggTable_SinceLastDays()
begin
	DECLARE done INT DEFAULT 0;
	DECLARE cur_clsuter_internal_name varchar(250);
	
	/* Iterate over all of the available clusters in cluster-memebrs */
	DECLARE clusters_iterator CURSOR FOR SELECT DISTINCT internal_name FROM cluster_members WHERE group_type='PhysicalMachine' and recorded_on>@last_stats;
	DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = 1;
	set @last_stats = IFNULL((select min(recorded_on) as mn from (select distinct recorded_on from cluster_members where recorded_on!=end_of_month(recorded_on)) as a), (select date(date_sub(now(), interval 60 day)) as d));
	...
	IF NOT done THEN
		call migrate_clusterAgg_Insert_SinceLastDays(cur_clsuter_internal_name);
	END IF;
...
/* MONTHLY */
delimiter //
drop procedure if exists migrate_clusterAgg_Insert_SinceLastMonths//
create procedure migrate_clusterAgg_Insert_SinceLastMonths(IN cluster_internal_name varchar(250))
begin

set @last_stats = IFNULL((select max(recorded_on) from cluster_stats_by_month 
	where convert(internal_name using utf8) = convert(cluster_internal_name using utf8)), 
(select date('2012-01-01') as d)); /* Default to beginning of the year */
 ...
 */ 
delimiter $$
drop procedure if exists migrate_populateAll_MonthlyClusterAggTable_SinceLastMonths$$
create procedure migrate_populateAll_MonthlyClusterAggTable_SinceLastMonths()
begin
	DECLARE done INT DEFAULT 0;
	DECLARE cur_clsuter_internal_name varchar(250);
	...
	FETCH clusters_iterator INTO cur_clsuter_internal_name;
	IF NOT done THEN
		call migrate_clusterAgg_Insert_SinceLastMonths(cur_clsuter_internal_name);
	END IF;
	UNTIL done END REPEAT;
...
/* PERFORM MIGRATION: */
call migrate_populateAll_DailyClusterAggTable_SinceLastDays();
call migrate_populateAll_MonthlyClusterAggTable_SinceLastMonths();
...
/*
 * Fix incorrect units in Mem-capacity entries
 */
update cluster_stats_by_month set value=((value/1024)/1024) where property_type='Mem' and property_subtype='capacity' and value>=100000;
update cluster_stats_by_day set value=((value/1024)/1024) where property_type='Mem' and property_subtype='capacity' and value>=100000; 
 

/*
 * Register version info:
 */
insert into version_info values (3,1) ;
