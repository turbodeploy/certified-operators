class VersionInfosController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @version_infos }
    end
  end

  def apply_to_checked_items
    if params[:action_to_perform] == 'Delete'    
        VersionInfo.destroy(params[:delchk].keys) if params[:delchk] && params[:delchk].keys.size > 0
    end 
  ...
  def replace_column_value
    ...
    VersionInfo.update_all("#{params[:field]} = '#{params[:new_value].strip}'", where_clause)
    redirect_to :back
  end
  ...
  def delete_matching_rows
    ...
    VersionInfo.delete_all(where_clause)
    redirect_to :back
  end
  ...
  def new
    @new_obj = @version_info = VersionInfo.new

    @ft_subject = "Version Info"
    @ft_activity = "New"
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @version_info }
    end
  end

  def set_context(load_related=true)
    @version_info = VersionInfo.find(params[:id])

    @ft_subject = "Version Info"
    @ft_activity = @version_info.iname
  ...
  def create
    sanitize_params(:version_info)
    @new_obj = @version_info = VersionInfo.new(params[:version_info])
    ...
    if ! ok_to_add_or_update?
      ...
      respond_to do |format|
        if @version_info.save
          format.xml  { render :xml => @version_info, :status => :created, :location => @version_info }
          format.html {
            flash[:notice] = "#{@version_info.iname} was successfully created."
            redirected_to_aggregator = update_aggregator(@version_info,params)
            if ( ! redirected_to_aggregator ) && session[:post_create_redirect]
                url = session[:post_create_redirect] + "?version_info_id=" + @version_info.id.to_s + "&version_info_ids[]=" + @version_info.id.to_s 
                redirect_to url
                session[:post_create_redirect] = nil
            else 
                redirect_to :id => @version_info.id, :action => (params[:commit].to_s.include?("Another") ? 'new' : 'edit') if ! redirected_to_aggregator 
            end
          }
        else
          format.xml  { render :xml => @version_info.errors, :status => :unprocessable_entity }
          format.html {
            flash[:notice] = 'Version Info could not be created. See errors.'
            render :action => 'new'
          }
        end
      end
  ...
  def update
    @updated_obj = @version_info = VersionInfo.find(params[:id])
    sanitize_params(:version_info)
    ...
    if ! ok_to_add_or_update?
      ...
      @ft_subject = "Version Info"
      @ft_activity = "Edit"
      redirect_to :action => 'edit', :id => @version_info and return
    end 

      respond_to do |format|
      if @version_info.update_attributes(params[:version_info])
        ...
        format.xml  { head :ok }
        format.html {
          flash[:notice] = 'Version Info was successfully updated.'
          redirect_to :action => 'show', :id => @version_info
        }
      else
        format.xml  { render :xml => @version_info.errors, :status => :unprocessable_entity }
        format.html { 
          redirect_to :action => 'edit', :id => @version_info
        }
      end
    end
  end
  ...
  def ok_to_add_or_update?
    if params[:version_info][:version].to_s.empty?
      @reason = 'The version field can not be blank.'
      return false
    end
    return true
  end
  ...
  def destroy
    obj = VersionInfo.find(params[:id])
    obj.destroy
    respond_to do |format|
      format.html {
        redirect_to :back
  ...
  def upload_csv
    ...
    #fn = "#{RAILS_ROOT}/uploads/version_info_data.csv"
    #File.open(fn, "wb") { |f| f.write(params[:csv_file].read) }
    #VersionInfo.load_csv_file(fn) if VersionInfo.respond_to? :load_csv_file

    VersionInfo.load_csv_str(params[:csv_file].read) if VersionInfo.respond_to? :load_csv_str
    
    redirect_to :action => 'list' 
  end

  def download_csv
    attr_names = VersionInfo.value_attrs
    ...
    send_data csv_string, :filename => 'version_info_data.csv', :type => 'text/csv'
  end 

  # --- 

  def show_queries
    @ft_subject = "VersionInfo"
    @ft_activity  = "Reports"

    @queries = VersionInfo.query_specs
  ...
  def run_query
    ...
    @page_clause = nil # nil # "LIMIT #{@page_size.to_s} OFFSET #{((@page_no-1)*@page_size).to_s}"

    @query = VersionInfo.query_specs[params[:query]]
    ...
    @count = VersionInfo.count_by_sql("SELECT COUNT(id) FROM (#{@base_sql}) AS row_data")
    
    @sql = @base_sql + (@sort_fields ? " ORDER BY #{@sort_fields}" : "") + (@page_clause ? " #{@page_clause}" : "") 
    @rows = VersionInfo.find_by_sql(apply_limit(@sql))

    @cols = @query[:cols] || (@rows.size > 0 && @rows[0].attributes.keys)

    @link_params = { :controller => 'version_infos', :action => 'show' }
  ...
  def prepare_list
    ...
    @ft_subject  = ( @ft_subject || "Version Info".pluralize )
    @ft_activity = ( @ft_activity || "All" )

    @order_field = ( params[:order_by] || "version" )
    ...
    adjust_sorting_and_filtering_params(VersionInfo,params)
    @order_field = params[:sort_field].to_s unless params[:sort_field].to_s.empty?

    @version_infos = run_query()
