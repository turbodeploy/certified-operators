class PmStatsByHoursController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @pm_stats_by_hours }
    end
  end

  def apply_to_checked_items
    if params[:action_to_perform] == 'Delete'    
        PmStatsByHour.destroy(params[:delchk].keys) if params[:delchk] && params[:delchk].keys.size > 0
    end 
  ...
  def replace_column_value
    ...
    PmStatsByHour.update_all("#{params[:field]} = '#{params[:new_value].strip}'", where_clause)
    redirect_to :back
  end
  ...
  def delete_matching_rows
    ...
    PmStatsByHour.delete_all(where_clause)
    redirect_to :back
  end
  ...
  def new
    @new_obj = @pm_stats_by_hour = PmStatsByHour.new

    @ft_subject = "Pm Stats By Hour"
    @ft_activity = "New"
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @pm_stats_by_hour }
    end
  end

  def set_context(load_related=true)
    @pm_stats_by_hour = PmStatsByHour.find(params[:id])

    @ft_subject = "Pm Stats By Hour"
    @ft_activity = @pm_stats_by_hour.iname
  ...
  def create
    sanitize_params(:pm_stats_by_hour)
    @new_obj = @pm_stats_by_hour = PmStatsByHour.new(params[:pm_stats_by_hour])
    ...
    if ! ok_to_add_or_update?
      ...
      respond_to do |format|
        if @pm_stats_by_hour.save
          format.xml  { render :xml => @pm_stats_by_hour, :status => :created, :location => @pm_stats_by_hour }
          format.html {
            flash[:notice] = "#{@pm_stats_by_hour.iname} was successfully created."
            redirected_to_aggregator = update_aggregator(@pm_stats_by_hour,params)
            if ( ! redirected_to_aggregator ) && session[:post_create_redirect]
                url = session[:post_create_redirect] + "?pm_stats_by_hour_id=" + @pm_stats_by_hour.id.to_s + "&pm_stats_by_hour_ids[]=" + @pm_stats_by_hour.id.to_s 
                redirect_to url
                session[:post_create_redirect] = nil
            else 
                redirect_to :id => @pm_stats_by_hour.id, :action => (params[:commit].to_s.include?("Another") ? 'new' : 'edit') if ! redirected_to_aggregator 
            end
          }
        else
          format.xml  { render :xml => @pm_stats_by_hour.errors, :status => :unprocessable_entity }
          format.html {
            flash[:notice] = 'Pm Stats By Hour could not be created. See errors.'
            render :action => 'new'
          }
        end
      end
  ...
  def update
    @updated_obj = @pm_stats_by_hour = PmStatsByHour.find(params[:id])
    sanitize_params(:pm_stats_by_hour)
    ...
    if ! ok_to_add_or_update?
      ...
      @ft_subject = "Pm Stats By Hour"
      @ft_activity = "Edit"
      redirect_to :action => 'edit', :id => @pm_stats_by_hour and return
    end 

      respond_to do |format|
      if @pm_stats_by_hour.update_attributes(params[:pm_stats_by_hour])
        ...
        format.xml  { head :ok }
        format.html {
          flash[:notice] = 'Pm Stats By Hour was successfully updated.'
          redirect_to :action => 'show', :id => @pm_stats_by_hour
        }
      else
        format.xml  { render :xml => @pm_stats_by_hour.errors, :status => :unprocessable_entity }
        format.html { 
          redirect_to :action => 'edit', :id => @pm_stats_by_hour
        }
      end
    end
  end
  ...
  def ok_to_add_or_update?
    if params[:pm_stats_by_hour][:snapshot_time].to_s.empty?
  ...
  def destroy
    obj = PmStatsByHour.find(params[:id])
    obj.destroy
    respond_to do |format|
      format.html {
        redirect_to :back
  ...
  def upload_csv
    ...
    #fn = "#{RAILS_ROOT}/uploads/pm_stats_by_hour_data.csv"
    #File.open(fn, "wb") { |f| f.write(params[:csv_file].read) }
    #PmStatsByHour.load_csv_file(fn) if PmStatsByHour.respond_to? :load_csv_file

    PmStatsByHour.load_csv_str(params[:csv_file].read) if PmStatsByHour.respond_to? :load_csv_str
    
    redirect_to :action => 'list' 
  end

  def download_csv
    attr_names = PmStatsByHour.value_attrs
    ...
    send_data csv_string, :filename => 'pm_stats_by_hour_data.csv', :type => 'text/csv'
  end 

  # --- 

  def show_queries
    @ft_subject = "PmStatsByHour"
    @ft_activity  = "Reports"

    @queries = PmStatsByHour.query_specs
  ...
  def run_query
    ...
    @query = PmStatsByHour.query_specs[params[:query]]
    ...
    @count = PmStatsByHour.count_by_sql("SELECT COUNT(*) FROM (#{@base_sql}) AS row_data")
    
    @sql = @base_sql + (@sort_fields ? " ORDER BY #{@sort_fields}" : "") + (@page_clause ? " #{@page_clause}" : "") 
    @rows = PmStatsByHour.find_by_sql(apply_limit(@sql))

    @cols = @query[:cols] || (@rows.size > 0 && @rows[0].attributes.keys)

    @link_params = { :controller => 'pm_stats_by_hours', :action => 'show' }
  ...
  def prepare_list
    ...
    @ft_subject  = ( @ft_subject || "Pm Stats By Hour".pluralize )
    ...
    adjust_sorting_and_filtering_params(PmStatsByHour,params)
    @order_field = params[:sort_field].to_s unless params[:sort_field].to_s.empty?

    @pm_stats_by_hours = run_query()
