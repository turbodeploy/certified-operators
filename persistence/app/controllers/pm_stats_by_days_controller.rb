class PmStatsByDaysController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @pm_stats_by_days }
    end
  end

  def apply_to_checked_items
    if params[:action_to_perform] == 'Delete'    
        PmStatsByDay.destroy(params[:delchk].keys) if params[:delchk] && params[:delchk].keys.size > 0
    end 
  ...
  def replace_column_value
    ...
    PmStatsByDay.update_all("#{params[:field]} = '#{params[:new_value].strip}'", where_clause)
    redirect_to :back
  end
  ...
  def delete_matching_rows
    ...
    PmStatsByDay.delete_all(where_clause)
    redirect_to :back
  end
  ...
  def new
    @new_obj = @pm_stats_by_day = PmStatsByDay.new

    @ft_subject = "Pm Stats By Day"
    @ft_activity = "New"
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @pm_stats_by_day }
    end
  end

  def set_context(load_related=true)
    @pm_stats_by_day = PmStatsByDay.find(params[:id])

    @ft_subject = "Pm Stats By Day"
    @ft_activity = @pm_stats_by_day.iname
  ...
  def create
    sanitize_params(:pm_stats_by_day)
    @new_obj = @pm_stats_by_day = PmStatsByDay.new(params[:pm_stats_by_day])
    ...
    if ! ok_to_add_or_update?
      ...
      respond_to do |format|
        if @pm_stats_by_day.save
          format.xml  { render :xml => @pm_stats_by_day, :status => :created, :location => @pm_stats_by_day }
          format.html {
            flash[:notice] = "#{@pm_stats_by_day.iname} was successfully created."
            redirected_to_aggregator = update_aggregator(@pm_stats_by_day,params)
            if ( ! redirected_to_aggregator ) && session[:post_create_redirect]
                url = session[:post_create_redirect] + "?pm_stats_by_day_id=" + @pm_stats_by_day.id.to_s + "&pm_stats_by_day_ids[]=" + @pm_stats_by_day.id.to_s 
                redirect_to url
                session[:post_create_redirect] = nil
            else 
                redirect_to :id => @pm_stats_by_day.id, :action => (params[:commit].to_s.include?("Another") ? 'new' : 'edit') if ! redirected_to_aggregator 
            end
          }
        else
          format.xml  { render :xml => @pm_stats_by_day.errors, :status => :unprocessable_entity }
          format.html {
            flash[:notice] = 'Pm Stats By Day could not be created. See errors.'
            render :action => 'new'
          }
        end
      end
  ...
  def update
    @updated_obj = @pm_stats_by_day = PmStatsByDay.find(params[:id])
    sanitize_params(:pm_stats_by_day)
    ...
    if ! ok_to_add_or_update?
      ...
      @ft_subject = "Pm Stats By Day"
      @ft_activity = "Edit"
      redirect_to :action => 'edit', :id => @pm_stats_by_day and return
    end 

      respond_to do |format|
      if @pm_stats_by_day.update_attributes(params[:pm_stats_by_day])
        ...
        format.xml  { head :ok }
        format.html {
          flash[:notice] = 'Pm Stats By Day was successfully updated.'
          redirect_to :action => 'show', :id => @pm_stats_by_day
        }
      else
        format.xml  { render :xml => @pm_stats_by_day.errors, :status => :unprocessable_entity }
        format.html { 
          redirect_to :action => 'edit', :id => @pm_stats_by_day
        }
      end
    end
  end
  ...
  def ok_to_add_or_update?
    if params[:pm_stats_by_day][:snapshot_time].to_s.empty?
  ...
  def destroy
    obj = PmStatsByDay.find(params[:id])
    obj.destroy
    respond_to do |format|
      format.html {
        redirect_to :back
  ...
  def upload_csv
    ...
    #fn = "#{RAILS_ROOT}/uploads/pm_stats_by_day_data.csv"
    #File.open(fn, "wb") { |f| f.write(params[:csv_file].read) }
    #PmStatsByDay.load_csv_file(fn) if PmStatsByDay.respond_to? :load_csv_file

    PmStatsByDay.load_csv_str(params[:csv_file].read) if PmStatsByDay.respond_to? :load_csv_str
    
    redirect_to :action => 'list' 
  end

  def download_csv
    attr_names = PmStatsByDay.value_attrs
    ...
    send_data csv_string, :filename => 'pm_stats_by_day_data.csv', :type => 'text/csv'
  end 

  # --- 

  def show_queries
    @ft_subject = "PmStatsByDay"
    @ft_activity  = "Reports"

    @queries = PmStatsByDay.query_specs
  ...
  def run_query
    ...
    @query = PmStatsByDay.query_specs[params[:query]]
    ...
    @count = PmStatsByDay.count_by_sql("SELECT COUNT(*) FROM (#{@base_sql}) AS row_data")
    
    @sql = @base_sql + (@sort_fields ? " ORDER BY #{@sort_fields}" : "") + (@page_clause ? " #{@page_clause}" : "") 
    @rows = PmStatsByDay.find_by_sql(apply_limit(@sql))

    @cols = @query[:cols] || (@rows.size > 0 && @rows[0].attributes.keys)

    @link_params = { :controller => 'pm_stats_by_days', :action => 'show' }
  ...
  def prepare_list
    ...
    @ft_subject  = ( @ft_subject || "Pm Stats By Day".pluralize )
    ...
    adjust_sorting_and_filtering_params(PmStatsByDay,params)
    @order_field = params[:sort_field].to_s unless params[:sort_field].to_s.empty?

    @pm_stats_by_days = run_query()
