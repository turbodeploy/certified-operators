class EntityAttrsController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @entity_attrs }
    end
  end

  def apply_to_checked_items
    if params[:action_to_perform] == 'Delete'    
        EntityAttr.destroy(params[:delchk].keys) if params[:delchk] && params[:delchk].keys.size > 0
    end 
  ...
  def replace_column_value
    ...
    EntityAttr.update_all("#{params[:field]} = '#{params[:new_value].strip}'", where_clause)
    redirect_to :back
  end
  ...
  def delete_matching_rows
    ...
    EntityAttr.delete_all(where_clause)
    redirect_to :back
  end
  ...
  def new
    @new_obj = @entity_attr = EntityAttr.new

    @ft_subject = "Entity Attr"
    @ft_activity = "New"
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @entity_attr }
    end
  end

  def set_context(load_related=true)
    @entity_attr = EntityAttr.find(params[:id])

    @ft_subject = "Entity Attr"
    @ft_activity = @entity_attr.iname
  ...
  def update_aggregator(new_obj,params)
    ...
    if @entity 
      @entity.attrs << new_obj
      unless session[:post_create_redirect]
        if ! params[:commit].to_s.include?("Another")
          redirect_to :controller => @entity.ctrlr_name, :action => 'edit', :id => @entity.id, :anchor => "attrs"
        else
  ...
  def create
    sanitize_params(:entity_attr)
    @new_obj = @entity_attr = EntityAttr.new(params[:entity_attr])
    ...
    if ! ok_to_add_or_update?
      ...
      respond_to do |format|
        if @entity_attr.save
          format.xml  { render :xml => @entity_attr, :status => :created, :location => @entity_attr }
          format.html {
            flash[:notice] = "#{@entity_attr.iname} was successfully created."
            redirected_to_aggregator = update_aggregator(@entity_attr,params)
            if ( ! redirected_to_aggregator ) && session[:post_create_redirect]
                url = session[:post_create_redirect] + "?entity_attr_id=" + @entity_attr.id.to_s + "&entity_attr_ids[]=" + @entity_attr.id.to_s 
                redirect_to url
                session[:post_create_redirect] = nil
            else 
                redirect_to :id => @entity_attr.id, :action => (params[:commit].to_s.include?("Another") ? 'new' : 'edit') if ! redirected_to_aggregator 
            end
          }
        else
          format.xml  { render :xml => @entity_attr.errors, :status => :unprocessable_entity }
          format.html {
            flash[:notice] = 'Entity Attr could not be created. See errors.'
            @entity = Entity.find(params[:entity_id]) if params[:entity_id] && Entity.exists?(params[:entity_id])
            render :action => 'new'
          }
        end
  ...
  def update
    @updated_obj = @entity_attr = EntityAttr.find(params[:id])
    sanitize_params(:entity_attr)
    ...
    if ! ok_to_add_or_update?
      ...
      @ft_subject = "Entity Attr"
      @ft_activity = "Edit"
      redirect_to :action => 'edit', :id => @entity_attr and return
    end 

      respond_to do |format|
      if @entity_attr.update_attributes(params[:entity_attr])
        ...
        format.xml  { head :ok }
        format.html {
          flash[:notice] = 'Entity Attr was successfully updated.'
          redirect_to :action => 'show', :id => @entity_attr
        }
      else
        format.xml  { render :xml => @entity_attr.errors, :status => :unprocessable_entity }
        format.html { 
          redirect_to :action => 'edit', :id => @entity_attr
        }
      end
    end
  end
  ...
  def ok_to_add_or_update?
    if params[:entity_attr][:name].to_s.empty?
  ...
  def upload_csv
    ...
    #fn = "#{RAILS_ROOT}/uploads/entity_attr_data.csv"
    #File.open(fn, "wb") { |f| f.write(params[:csv_file].read) }
    #EntityAttr.load_csv_file(fn) if EntityAttr.respond_to? :load_csv_file

    EntityAttr.load_csv_str(params[:csv_file].read) if EntityAttr.respond_to? :load_csv_str
    
    redirect_to :action => 'list' 
  end

  def download_csv
    attr_names = EntityAttr.value_attrs
    ...
    send_data csv_string, :filename => 'entity_attr_data.csv', :type => 'text/csv'
  end 

  # --- 

  def show_queries
    @ft_subject = "EntityAttr"
    @ft_activity  = "Reports"

    @queries = EntityAttr.query_specs
  ...
  def run_query
    ...
    @query = EntityAttr.query_specs[params[:query]]
    ...
    @count = EntityAttr.count_by_sql("SELECT COUNT(id) FROM (#{@base_sql}) AS row_data")
    
    @sql = @base_sql + (@sort_fields ? " ORDER BY #{@sort_fields}" : "") + (@page_clause ? " #{@page_clause}" : "") 
    @rows = EntityAttr.find_by_sql(apply_limit(@sql))

    @cols = @query[:cols] || (@rows.size > 0 && @rows[0].attributes.keys)

    @link_params = { :controller => 'entity_attrs', :action => 'show' }
  ...
  def prepare_list
    ...
    @ft_subject  = ( @ft_subject || "Entity Attr".pluralize )
    ...
    adjust_sorting_and_filtering_params(EntityAttr,params)
    @order_field = params[:sort_field].to_s unless params[:sort_field].to_s.empty?

    @entity_attrs = run_query()
