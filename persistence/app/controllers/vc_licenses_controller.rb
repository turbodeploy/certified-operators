class VcLicensesController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @vc_licenses }
    end
  end

  def apply_to_checked_items
    if params[:action_to_perform] == 'Delete'    
        VcLicense.destroy(params[:delchk].keys) if params[:delchk] && params[:delchk].keys.size > 0
    end 
  ...
  def replace_column_value
    ...
    VcLicense.update_all("#{params[:field]} = '#{params[:new_value].strip}'", where_clause)
    redirect_to :back
  end
  ...
  def delete_matching_rows
    ...
    VcLicense.delete_all(where_clause)
    redirect_to :back
  end
  ...
  def quick_add_items
    ...
    if str_names.length > 0 then
      ...
      for str_name in names do
        str_name.strip!
        VcLicense.find_or_create_by_target(str_name)
      end
    end
  ...
  def new
    @new_obj = @vc_license = VcLicense.new

    @ft_subject = "Vc License"
    @ft_activity = "New"
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @vc_license }
    end
  end

  def set_context(load_related=true)
    @vc_license = VcLicense.find(params[:id])

    @ft_subject = "Vc License"
    @ft_activity = @vc_license.iname
  ...
  def create
    sanitize_params(:vc_license)
    @new_obj = @vc_license = VcLicense.new(params[:vc_license])
    ...
    if ! ok_to_add_or_update?
      ...
      respond_to do |format|
        if @vc_license.save
          format.xml  { render :xml => @vc_license, :status => :created, :location => @vc_license }
          format.html {
            flash[:notice] = "#{@vc_license.iname} was successfully created."
            redirected_to_aggregator = update_aggregator(@vc_license,params)
            if ( ! redirected_to_aggregator ) && session[:post_create_redirect]
                url = session[:post_create_redirect] + "?vc_license_id=" + @vc_license.id.to_s + "&vc_license_ids[]=" + @vc_license.id.to_s 
                redirect_to url
                session[:post_create_redirect] = nil
            else 
                redirect_to :id => @vc_license.id, :action => (params[:commit].to_s.include?("Another") ? 'new' : 'edit') if ! redirected_to_aggregator 
            end
          }
        else
          format.xml  { render :xml => @vc_license.errors, :status => :unprocessable_entity }
          format.html {
            flash[:notice] = 'Vc License could not be created. See errors.'
            render :action => 'new'
          }
        end
      end
  ...
  def update
    @updated_obj = @vc_license = VcLicense.find(params[:id])
    sanitize_params(:vc_license)
    ...
    if ! ok_to_add_or_update?
      ...
      @ft_subject = "Vc License"
      @ft_activity = "Edit"
      redirect_to :action => 'edit', :id => @vc_license and return
    end 

      respond_to do |format|
      if @vc_license.update_attributes(params[:vc_license])
        ...
        format.xml  { head :ok }
        format.html {
          flash[:notice] = 'Vc License was successfully updated.'
          redirect_to :action => 'show', :id => @vc_license
        }
      else
        format.xml  { render :xml => @vc_license.errors, :status => :unprocessable_entity }
        format.html { 
          redirect_to :action => 'edit', :id => @vc_license
        }
      end
    end
  end
  ...
  def ok_to_add_or_update?
    if params[:vc_license][:target].to_s.empty?
      @reason = 'The target field can not be blank.'
      return false
    end
    return true
  end
  ...
  def upload_csv
    ...
    #fn = "#{RAILS_ROOT}/uploads/vc_license_data.csv"
    #File.open(fn, "wb") { |f| f.write(params[:csv_file].read) }
    #VcLicense.load_csv_file(fn) if VcLicense.respond_to? :load_csv_file

    VcLicense.load_csv_str(params[:csv_file].read) if VcLicense.respond_to? :load_csv_str
    
    redirect_to :action => 'list' 
  end

  def download_csv
    attr_names = VcLicense.value_attrs
    ...
    send_data csv_string, :filename => 'vc_license_data.csv', :type => 'text/csv'
  end 

  # --- 

  def show_queries
    @ft_subject = "VcLicense"
    @ft_activity  = "Reports"

    @queries = VcLicense.query_specs
  ...
  def run_query
    ...
    @query = VcLicense.query_specs[params[:query]]
    ...
    @count = VcLicense.count_by_sql("SELECT COUNT(id) FROM (#{@base_sql}) AS row_data")
    
    @sql = @base_sql + (@sort_fields ? " ORDER BY #{@sort_fields}" : "") + (@page_clause ? " #{@page_clause}" : "") 
    @rows = VcLicense.find_by_sql(apply_limit(@sql))

    @cols = @query[:cols] || (@rows.size > 0 && @rows[0].attributes.keys)

    @link_params = { :controller => 'vc_licenses', :action => 'show' }
  ...
  def prepare_list
    ...
    @ft_subject  = ( @ft_subject || "Vc License".pluralize )
    @ft_activity = ( @ft_activity || "All" )

    @order_field = ( params[:order_by] || "target" )
    ...
    adjust_sorting_and_filtering_params(VcLicense,params)
    @order_field = params[:sort_field].to_s unless params[:sort_field].to_s.empty?

    @vc_licenses = run_query()
