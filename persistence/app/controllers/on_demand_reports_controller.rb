class OnDemandReportsController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @on_demand_reports }
    end
  end

  def apply_to_checked_items
    if params[:action_to_perform] == 'Delete'    
        OnDemandReport.destroy(params[:delchk].keys) if params[:delchk] && params[:delchk].keys.size > 0
    end 
  ...
  def replace_column_value
    ...
    OnDemandReport.update_all("#{params[:field]} = '#{params[:new_value].strip}'", where_clause)
    redirect_to :back
  end
  ...
  def delete_matching_rows
    ...
    OnDemandReport.delete_all(where_clause)
    redirect_to :back
  end
  ...
  def quick_add_items
    ...
    if str_names.length > 0 then
      ...
      for str_name in names do
        str_name.strip!
        OnDemandReport.find_or_create_by_filename(str_name)
      end
    end
  ...
  def new
    @new_obj = @on_demand_report = OnDemandReport.new

    @ft_subject = "On Demand Report"
    @ft_activity = "New"
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @on_demand_report }
    end
  end

  def set_context(load_related=true)
    @on_demand_report = OnDemandReport.find(params[:id])

    @ft_subject = "On Demand Report"
    @ft_activity = @on_demand_report.iname
    ...
    @order_field = get_order_field(params,'on_demand_report','report_subscriptions','email')
    @id_set = @on_demand_report.send("#{'report_subscriptions'.singularize}_ids").concat(["0"]).join(",")
  ...
  def create
    sanitize_params(:on_demand_report)
    @new_obj = @on_demand_report = OnDemandReport.new(params[:on_demand_report])
    ...
    if ! ok_to_add_or_update?
      ...
      respond_to do |format|
        if @on_demand_report.save
          format.xml  { render :xml => @on_demand_report, :status => :created, :location => @on_demand_report }
          format.html {
            flash[:notice] = "#{@on_demand_report.iname} was successfully created."
            redirected_to_aggregator = update_aggregator(@on_demand_report,params)
            if ( ! redirected_to_aggregator ) && session[:post_create_redirect]
                url = session[:post_create_redirect] + "?on_demand_report_id=" + @on_demand_report.id.to_s + "&on_demand_report_ids[]=" + @on_demand_report.id.to_s 
                redirect_to url
                session[:post_create_redirect] = nil
            else 
                redirect_to :id => @on_demand_report.id, :action => (params[:commit].to_s.include?("Another") ? 'new' : 'edit') if ! redirected_to_aggregator 
            end
          }
        else
          format.xml  { render :xml => @on_demand_report.errors, :status => :unprocessable_entity }
          format.html {
            flash[:notice] = 'On Demand Report could not be created. See errors.'
            render :action => 'new'
          }
        end
      end
  ...
  def update
    @updated_obj = @on_demand_report = OnDemandReport.find(params[:id])
    sanitize_params(:on_demand_report)
    ...
    if ! ok_to_add_or_update?
      ...
      @ft_subject = "On Demand Report"
      @ft_activity = "Edit"
      redirect_to :action => 'edit', :id => @on_demand_report and return
    end 

      respond_to do |format|
      if @on_demand_report.update_attributes(params[:on_demand_report])
        ...
        format.xml  { head :ok }
        format.html {
          flash[:notice] = 'On Demand Report was successfully updated.'
          redirect_to :action => 'show', :id => @on_demand_report
        }
      else
        format.xml  { render :xml => @on_demand_report.errors, :status => :unprocessable_entity }
        format.html { 
          redirect_to :action => 'edit', :id => @on_demand_report
        }
      end
    end
  end
  ...
  def ok_to_add_or_update?
    if params[:on_demand_report][:filename].to_s.empty?
  ...
  def destroy
    obj = OnDemandReport.find(params[:id])
    obj.destroy
    respond_to do |format|
      format.html {
        redirect_to :back
  ...
  def add_to_report_subscriptions
    ...
    @on_demand_report = OnDemandReport.find(params[:id])
    @report_subscriptions = ReportSubscription.choices_for(@on_demand_report,'report_subscriptions')
    @report_subscriptions = @report_subscriptions - [ @on_demand_report ]
    @my_report_subscriptions = @on_demand_report.report_subscriptions
  ...
  def update_report_subscriptions
    ...
    @on_demand_report.report_subscriptions << @report_subscriptions if @report_subscriptions
    redirect_to :controller => @on_demand_report.ctrlr_name, :action => 'edit', :id => @on_demand_report, :anchor => 'report_subscriptions'
  end

  def remove_from_report_subscriptions
    @on_demand_report = OnDemandReport.find(params[:id])
    @report_subscriptions = ReportSubscription.find(params[:report_subscription_id])
    @on_demand_report.report_subscriptions.delete @report_subscriptions
    redirect_to :controller => @on_demand_report.ctrlr_name, :action => 'edit', :id => @on_demand_report, :anchor => 'report_subscriptions'
  end
  ...
  def quick_add_report_subscriptions
    ...
    if str_names.length > 0 then
      names = str_names.split("\n")
      existing_report_subscriptions = @on_demand_report.report_subscriptions.find(:all)
      for str_name in names do
        str_name.strip!
        new_obj = ReportSubscription.find_or_create_by_email(str_name)
        @on_demand_report.report_subscriptions << new_obj unless existing_report_subscriptions.include?(new_obj)
      end
    end
    
    redirect_to :action => 'edit', :id => @on_demand_report.id
  end
  ...
  def upload_csv
    ...
    #fn = "#{RAILS_ROOT}/uploads/on_demand_report_data.csv"
    #File.open(fn, "wb") { |f| f.write(params[:csv_file].read) }
    #OnDemandReport.load_csv_file(fn) if OnDemandReport.respond_to? :load_csv_file

    OnDemandReport.load_csv_str(params[:csv_file].read) if OnDemandReport.respond_to? :load_csv_str
    
    redirect_to :action => 'list' 
  end

  def download_csv
    attr_names = OnDemandReport.value_attrs
    ...
    send_data csv_string, :filename => 'on_demand_report_data.csv', :type => 'text/csv'
  end 

  # --- 

  def show_queries
    @ft_subject = "OnDemandReport"
    @ft_activity  = "Reports"

    @queries = OnDemandReport.query_specs
  ...
  def run_query
    ...
    @query = OnDemandReport.query_specs[params[:query]]
    ...
    @count = OnDemandReport.count_by_sql("SELECT COUNT(id) FROM (#{@base_sql}) AS row_data")
    
    @sql = @base_sql + (@sort_fields ? " ORDER BY #{@sort_fields}" : "") + (@page_clause ? " #{@page_clause}" : "") 
    @rows = OnDemandReport.find_by_sql(apply_limit(@sql))

    @cols = @query[:cols] || (@rows.size > 0 && @rows[0].attributes.keys)

    @link_params = { :controller => 'on_demand_reports', :action => 'show' }
  ...
  def prepare_list
    ...
    @ft_subject  = ( @ft_subject || "On Demand Report".pluralize )
    ...
    adjust_sorting_and_filtering_params(OnDemandReport,params)
    @order_field = params[:sort_field].to_s unless params[:sort_field].to_s.empty?

    @on_demand_reports = run_query()
