class NotificationsController < ApplicationController
  ...
  def apply_to_checked_items
    if params[:action_to_perform] == 'Delete'    
        Notification.destroy(params[:delchk].keys) if params[:delchk] && params[:delchk].keys.size > 0
    end 
  ...
  def replace_column_value
    ...
    Notification.update_all("#{params[:field]} = '#{params[:new_value].strip}'", where_clause)
    redirect_to :back
  end
  ...
  def delete_matching_rows
    ...
    Notification.delete_all(where_clause)
    redirect_to :back
  end
  ...
  def new
    @new_obj = @notification = Notification.new

    @ft_subject = "Notification"
    @ft_activity = "New"
  ...
  def set_context(load_related=true)
    ...
    @ft_subject = "Notification"
    @ft_activity = @notification.iname
  ...
  def create
    sanitize_params(:notification)
    @new_obj = @notification = Notification.new(params[:notification])
    ...
    if ! ok_to_add_or_update?
      ...
      respond_to do |format|
        if @notification.save
          ...
          format.html {
            flash[:notice] = "#{@notification.iname} was successfully created."
            redirected_to_aggregator = update_aggregator(@notification,params)
            if ( ! redirected_to_aggregator ) && session[:post_create_redirect]
                url = session[:post_create_redirect] + "?notification_id=" + @notification.id.to_s + "&notification_ids[]=" + @notification.id.to_s 
                redirect_to url
                session[:post_create_redirect] = nil
            else 
                redirect_to :id => @notification.id, :action => (params[:commit].to_s.include?("Another") ? 'new' : 'edit') if ! redirected_to_aggregator 
            end
          }
        else
          format.xml  { render :xml => @notification.errors, :status => :unprocessable_entity }
          format.html {
            flash[:notice] = 'Notification could not be created. See errors.'
            render :action => 'new'
          }
        end
      end
  ...
  def update
    @updated_obj = @notification = Notification.find(params[:id])
    sanitize_params(:notification)
    ...
    if ! ok_to_add_or_update?
      ...
      @ft_subject = "Notification"
      @ft_activity = "Edit"
      redirect_to :action => 'edit', :id => @notification and return
    end 
      ...
      if @notification.update_attributes(params[:notification])
        ...
        format.html {
          ...
          redirect_to :action => 'show', :id => @notification
        }
      else
        format.xml  { render :xml => @notification.errors, :status => :unprocessable_entity }
        format.html { 
          redirect_to :action => 'edit', :id => @notification
        }
      end
    end
  end
  ...
  def ok_to_add_or_update?
    if params[:notification][:clear_time].to_s.empty?
      @reason = 'The clear_time field can not be blank.'
      return false
    end
    return true
  end
  ...
  def upload_csv
    ...
    #fn = "#{RAILS_ROOT}/uploads/notification_data.csv"
    #File.open(fn, "wb") { |f| f.write(params[:csv_file].read) }
    #Notification.load_csv_file(fn) if Notification.respond_to? :load_csv_file

    Notification.load_csv_str(params[:csv_file].read) if Notification.respond_to? :load_csv_str
    
    redirect_to :action => 'list' 
  end

  def download_csv
    attr_names = Notification.value_attrs
    ...
    send_data csv_string, :filename => 'notification_data.csv', :type => 'text/csv'
  end 
  ...
  def show_queries
    ...
    @queries = Notification.query_specs
  ...
  def run_query
    ...
    @query = Notification.query_specs[params[:query]]
    ...
    @count = Notification.count_by_sql("SELECT COUNT(id) FROM (#{@base_sql}) AS row_data")
    
    @sql = @base_sql + (@sort_fields ? " ORDER BY #{@sort_fields}" : "") + (@page_clause ? " #{@page_clause}" : "") 
    @rows = Notification.find_by_sql(apply_limit(@sql))

    @cols = @query[:cols] || (@rows.size > 0 && @rows[0].attributes.keys)

    @link_params = { :controller => 'notifications', :action => 'show' }
  ...
  def prepare_list
    ...
    @ft_subject  = ( @ft_subject || "Notification".pluralize )
    @ft_activity = ( @ft_activity || "All" )

    @order_field = ( params[:order_by] || "clear_time" )
    ...
    adjust_sorting_and_filtering_params(Notification,params)
