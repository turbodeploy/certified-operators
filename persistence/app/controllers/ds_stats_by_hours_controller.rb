class DsStatsByHoursController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @ds_stats_by_hours }
    end
  end

  def apply_to_checked_items
    if params[:action_to_perform] == 'Delete'    
        DsStatsByHour.destroy(params[:delchk].keys) if params[:delchk] && params[:delchk].keys.size > 0
    end 
  ...
  def replace_column_value
    ...
    DsStatsByHour.update_all("#{params[:field]} = '#{params[:new_value].strip}'", where_clause)
    redirect_to :back
  end
  ...
  def delete_matching_rows
    ...
    DsStatsByHour.delete_all(where_clause)
    redirect_to :back
  end
  ...
  def new
    @new_obj = @ds_stats_by_hour = DsStatsByHour.new

    @ft_subject = "Ds Stats By Hour"
    @ft_activity = "New"
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @ds_stats_by_hour }
    end
  end

  def set_context(load_related=true)
    @ds_stats_by_hour = DsStatsByHour.find(params[:id])

    @ft_subject = "Ds Stats By Hour"
    @ft_activity = @ds_stats_by_hour.iname
  ...
  def create
    sanitize_params(:ds_stats_by_hour)
    @new_obj = @ds_stats_by_hour = DsStatsByHour.new(params[:ds_stats_by_hour])
    ...
    if ! ok_to_add_or_update?
      ...
      respond_to do |format|
        if @ds_stats_by_hour.save
          format.xml  { render :xml => @ds_stats_by_hour, :status => :created, :location => @ds_stats_by_hour }
          format.html {
            flash[:notice] = "#{@ds_stats_by_hour.iname} was successfully created."
            redirected_to_aggregator = update_aggregator(@ds_stats_by_hour,params)
            if ( ! redirected_to_aggregator ) && session[:post_create_redirect]
                url = session[:post_create_redirect] + "?ds_stats_by_hour_id=" + @ds_stats_by_hour.id.to_s + "&ds_stats_by_hour_ids[]=" + @ds_stats_by_hour.id.to_s 
                redirect_to url
                session[:post_create_redirect] = nil
            else 
                redirect_to :id => @ds_stats_by_hour.id, :action => (params[:commit].to_s.include?("Another") ? 'new' : 'edit') if ! redirected_to_aggregator 
            end
          }
        else
          format.xml  { render :xml => @ds_stats_by_hour.errors, :status => :unprocessable_entity }
          format.html {
            flash[:notice] = 'Ds Stats By Hour could not be created. See errors.'
            render :action => 'new'
          }
        end
      end
  ...
  def update
    @updated_obj = @ds_stats_by_hour = DsStatsByHour.find(params[:id])
    sanitize_params(:ds_stats_by_hour)
    ...
    if ! ok_to_add_or_update?
      ...
      @ft_subject = "Ds Stats By Hour"
      @ft_activity = "Edit"
      redirect_to :action => 'edit', :id => @ds_stats_by_hour and return
    end 

      respond_to do |format|
      if @ds_stats_by_hour.update_attributes(params[:ds_stats_by_hour])
        ...
        format.xml  { head :ok }
        format.html {
          flash[:notice] = 'Ds Stats By Hour was successfully updated.'
          redirect_to :action => 'show', :id => @ds_stats_by_hour
        }
      else
        format.xml  { render :xml => @ds_stats_by_hour.errors, :status => :unprocessable_entity }
        format.html { 
          redirect_to :action => 'edit', :id => @ds_stats_by_hour
        }
      end
    end
  end
  ...
  def ok_to_add_or_update?
    if params[:ds_stats_by_hour][:snapshot_time].to_s.empty?
  ...
  def destroy
    obj = DsStatsByHour.find(params[:id])
    obj.destroy
    respond_to do |format|
      format.html {
        redirect_to :back
  ...
  def upload_csv
    ...
    #fn = "#{RAILS_ROOT}/uploads/ds_stats_by_hour_data.csv"
    #File.open(fn, "wb") { |f| f.write(params[:csv_file].read) }
    #DsStatsByHour.load_csv_file(fn) if DsStatsByHour.respond_to? :load_csv_file

    DsStatsByHour.load_csv_str(params[:csv_file].read) if DsStatsByHour.respond_to? :load_csv_str
    
    redirect_to :action => 'list' 
  end

  def download_csv
    attr_names = DsStatsByHour.value_attrs
    ...
    send_data csv_string, :filename => 'ds_stats_by_hour_data.csv', :type => 'text/csv'
  end 

  # --- 

  def show_queries
    @ft_subject = "DsStatsByHour"
    @ft_activity  = "Reports"

    @queries = DsStatsByHour.query_specs
  ...
  def run_query
    ...
    @query = DsStatsByHour.query_specs[params[:query]]
    ...
    @count = DsStatsByHour.count_by_sql("SELECT COUNT(*) FROM (#{@base_sql}) AS row_data")
    
    @sql = @base_sql + (@sort_fields ? " ORDER BY #{@sort_fields}" : "") + (@page_clause ? " #{@page_clause}" : "") 
    @rows = DsStatsByHour.find_by_sql(apply_limit(@sql))

    @cols = @query[:cols] || (@rows.size > 0 && @rows[0].attributes.keys)

    @link_params = { :controller => 'ds_stats_by_hours', :action => 'show' }
  ...
  def prepare_list
    ...
    @ft_subject  = ( @ft_subject || "Ds Stats By Hour".pluralize )
    ...
    adjust_sorting_and_filtering_params(DsStatsByHour,params)
    @order_field = params[:sort_field].to_s unless params[:sort_field].to_s.empty?

    @ds_stats_by_hours = run_query()
