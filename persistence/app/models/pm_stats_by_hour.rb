...
class PmStatsByHour < ActiveRecord::Base

   include FtUtils

   set_table_name 'pm_stats_by_hour'
 ...
 public
  ...
  def next_item
    item = PmStatsByHour.find(:first, :order => FtUtils.quoted_name('snapshot_time'), :conditions => ["snapshot_time > ? ", self.snapshot_time] )
    item = PmStatsByHour.find(:first, :order => FtUtils.quoted_name('snapshot_time')) if item.nil? 
    return item
  end

  def prev_item
    item = PmStatsByHour.find(:first, :order => FtUtils.quoted_name('snapshot_time') + ' DESC', :conditions => ["snapshot_time < ? ", self.snapshot_time] )
    item = PmStatsByHour.find(:first, :order => FtUtils.quoted_name('snapshot_time') + ' DESC') if item.nil? 
    return item
  end

  def self.items_for_index
    return PmStatsByHour.find(:all, :order => 'snapshot_time')
  end
  ...
  def self.default_query(cond=nil)
    ...
    return <<-EOS
          (SELECT pm_stats_by_hour.snapshot_time as #{FtUtils.quoted_name('snapshot_time')},
              pm_stats_by_hour.uuid as #{FtUtils.quoted_name('uuid')},
              pm_stats_by_hour.producer_uuid as #{FtUtils.quoted_name('producer_uuid')},
              pm_stats_by_hour.property_type as #{FtUtils.quoted_name('property_type')},
              pm_stats_by_hour.property_subtype as #{FtUtils.quoted_name('property_subtype')},
              pm_stats_by_hour.capacity as #{FtUtils.quoted_name('capacity')},
              pm_stats_by_hour.avg_value as #{FtUtils.quoted_name('avg_value')},
              pm_stats_by_hour.min_value as #{FtUtils.quoted_name('min_value')},
              pm_stats_by_hour.max_value as #{FtUtils.quoted_name('max_value')},
              pm_stats_by_hour.std_dev as #{FtUtils.quoted_name('std_dev')}
          FROM
              #{FtUtils.quoted_name('pm_stats_by_hour')} AS #{FtUtils.quoted_name('pm_stats_by_hour')}
          #{cond ? ("WHERE "+cond.to_s) : ""}
          ) AS pm_stats_by_hours
          EOS
  end
  ...
  def self.query_specs
    ...
    @@query_specs = {
      "list_all" => 
        { :title => "List of "+"Pm Stats By Hour".pluralize,
          :sql  => "SELECT * FROM #{@@default_query}",
          :cols => PmStatsByHour.value_attrs.concat(PmStatsByHour.foreign_key_attrs),
