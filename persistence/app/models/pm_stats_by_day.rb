...
class PmStatsByDay < ActiveRecord::Base

   include FtUtils

   set_table_name 'pm_stats_by_day'
 ...
 public
  ...
  def next_item
    item = PmStatsByDay.find(:first, :order => FtUtils.quoted_name('snapshot_time'), :conditions => ["snapshot_time > ? ", self.snapshot_time] )
    item = PmStatsByDay.find(:first, :order => FtUtils.quoted_name('snapshot_time')) if item.nil? 
    return item
  end

  def prev_item
    item = PmStatsByDay.find(:first, :order => FtUtils.quoted_name('snapshot_time') + ' DESC', :conditions => ["snapshot_time < ? ", self.snapshot_time] )
    item = PmStatsByDay.find(:first, :order => FtUtils.quoted_name('snapshot_time') + ' DESC') if item.nil? 
    return item
  end

  def self.items_for_index
    return PmStatsByDay.find(:all, :order => 'snapshot_time')
  end
  ...
  def self.default_query(cond=nil)
    ...
    return <<-EOS
          (SELECT pm_stats_by_day.snapshot_time as #{FtUtils.quoted_name('snapshot_time')},
              pm_stats_by_day.uuid as #{FtUtils.quoted_name('uuid')},
              pm_stats_by_day.producer_uuid as #{FtUtils.quoted_name('producer_uuid')},
              pm_stats_by_day.property_type as #{FtUtils.quoted_name('property_type')},
              pm_stats_by_day.property_subtype as #{FtUtils.quoted_name('property_subtype')},
              pm_stats_by_day.capacity as #{FtUtils.quoted_name('capacity')},
              pm_stats_by_day.avg_value as #{FtUtils.quoted_name('avg_value')},
              pm_stats_by_day.min_value as #{FtUtils.quoted_name('min_value')},
              pm_stats_by_day.max_value as #{FtUtils.quoted_name('max_value')},
              pm_stats_by_day.std_dev as #{FtUtils.quoted_name('std_dev')}
          FROM
              #{FtUtils.quoted_name('pm_stats_by_day')} AS #{FtUtils.quoted_name('pm_stats_by_day')}
  ...
  def self.query_specs
    ...
    @@query_specs = {
      "list_all" => 
        { :title => "List of "+"Pm Stats By Day".pluralize,
          :sql  => "SELECT * FROM #{@@default_query}",
          :cols => PmStatsByDay.value_attrs.concat(PmStatsByDay.foreign_key_attrs),
