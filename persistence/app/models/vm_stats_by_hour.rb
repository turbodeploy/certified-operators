...
class VmStatsByHour < ActiveRecord::Base

   include FtUtils

   set_table_name 'vm_stats_by_hour'
 ...
 public
  ...
  def next_item
    item = VmStatsByHour.find(:first, :order => FtUtils.quoted_name('snapshot_time'), :conditions => ["snapshot_time > ? ", self.snapshot_time] )
    item = VmStatsByHour.find(:first, :order => FtUtils.quoted_name('snapshot_time')) if item.nil? 
    return item
  end

  def prev_item
    item = VmStatsByHour.find(:first, :order => FtUtils.quoted_name('snapshot_time') + ' DESC', :conditions => ["snapshot_time < ? ", self.snapshot_time] )
    item = VmStatsByHour.find(:first, :order => FtUtils.quoted_name('snapshot_time') + ' DESC') if item.nil? 
    return item
  end

  def self.items_for_index
    return VmStatsByHour.find(:all, :order => 'snapshot_time')
  end
  ...
  def self.default_query(cond=nil)
    ...
    return <<-EOS
          (SELECT vm_stats_by_hour.snapshot_time as #{FtUtils.quoted_name('snapshot_time')},
              vm_stats_by_hour.uuid as #{FtUtils.quoted_name('uuid')},
              vm_stats_by_hour.producer_uuid as #{FtUtils.quoted_name('producer_uuid')},
              vm_stats_by_hour.property_type as #{FtUtils.quoted_name('property_type')},
              vm_stats_by_hour.property_subtype as #{FtUtils.quoted_name('property_subtype')},
              vm_stats_by_hour.capacity as #{FtUtils.quoted_name('capacity')},
              vm_stats_by_hour.avg_value as #{FtUtils.quoted_name('avg_value')},
              vm_stats_by_hour.min_value as #{FtUtils.quoted_name('min_value')},
              vm_stats_by_hour.max_value as #{FtUtils.quoted_name('max_value')},
              vm_stats_by_hour.std_dev as #{FtUtils.quoted_name('std_dev')}
          FROM
              #{FtUtils.quoted_name('vm_stats_by_hour')} AS #{FtUtils.quoted_name('vm_stats_by_hour')}
          #{cond ? ("WHERE "+cond.to_s) : ""}
          ) AS vm_stats_by_hours
          EOS
  end
  ...
  def self.query_specs
    ...
    @@query_specs = {
      "list_all" => 
        { :title => "List of "+"Vm Stats By Hour".pluralize,
          :sql  => "SELECT * FROM #{@@default_query}",
          :cols => VmStatsByHour.value_attrs.concat(VmStatsByHour.foreign_key_attrs),
