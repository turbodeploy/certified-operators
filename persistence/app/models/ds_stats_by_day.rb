...
class DsStatsByDay < ActiveRecord::Base

   include FtUtils

   set_table_name 'ds_stats_by_day'
 ...
 public
  ...
  def next_item
    item = DsStatsByDay.find(:first, :order => FtUtils.quoted_name('snapshot_time'), :conditions => ["snapshot_time > ? ", self.snapshot_time] )
    item = DsStatsByDay.find(:first, :order => FtUtils.quoted_name('snapshot_time')) if item.nil? 
    return item
  end

  def prev_item
    item = DsStatsByDay.find(:first, :order => FtUtils.quoted_name('snapshot_time') + ' DESC', :conditions => ["snapshot_time < ? ", self.snapshot_time] )
    item = DsStatsByDay.find(:first, :order => FtUtils.quoted_name('snapshot_time') + ' DESC') if item.nil? 
    return item
  end

  def self.items_for_index
    return DsStatsByDay.find(:all, :order => 'snapshot_time')
  end
  ...
  def self.default_query(cond=nil)
    ...
    return <<-EOS
          (SELECT ds_stats_by_day.snapshot_time as #{FtUtils.quoted_name('snapshot_time')},
              ds_stats_by_day.uuid as #{FtUtils.quoted_name('uuid')},
              ds_stats_by_day.producer_uuid as #{FtUtils.quoted_name('producer_uuid')},
              ds_stats_by_day.property_type as #{FtUtils.quoted_name('property_type')},
              ds_stats_by_day.property_subtype as #{FtUtils.quoted_name('property_subtype')},
              ds_stats_by_day.capacity as #{FtUtils.quoted_name('capacity')},
              ds_stats_by_day.avg_value as #{FtUtils.quoted_name('avg_value')},
              ds_stats_by_day.min_value as #{FtUtils.quoted_name('min_value')},
              ds_stats_by_day.max_value as #{FtUtils.quoted_name('max_value')},
              ds_stats_by_day.std_dev as #{FtUtils.quoted_name('std_dev')}
          FROM
              #{FtUtils.quoted_name('ds_stats_by_day')} AS #{FtUtils.quoted_name('ds_stats_by_day')}
  ...
  def self.query_specs
    ...
    @@query_specs = {
      "list_all" => 
        { :title => "List of "+"Ds Stats By Day".pluralize,
          :sql  => "SELECT * FROM #{@@default_query}",
          :cols => DsStatsByDay.value_attrs.concat(DsStatsByDay.foreign_key_attrs),
