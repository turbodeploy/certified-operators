...
public class PlanProgressListener implements ActionsListener, RepositoryListener, StatsListener,
    ...
    public PlanProgressListener(@Nonnull final PlanDao planDao,
        ...
        this.planAccess = new PlanAccess(Objects.requireNonNull(planDao));
    ...
    public void onTopologySummary(@Nonnull TopologySummary topologySummary) {
        ...
        try {
            final PlanInstance plan = planAccess.updatePlanProgress(planId, planBuilder -> {
    ...
    public void onActionsUpdated(@Nonnull final ActionsUpdated actionsUpdated) {
        ...
        if (planId != realtimeTopologyContextId) {
            try {
                Optional<PlanInstance> planInstance = planAccess.getPlan(planId);
                if (!planInstance.isPresent()) {
                    ...
                    if (actionsUpdated.hasUpdateFailure()) {
                        ...
                        planAccess.updatePlanProgress(planId, planBuilder -> {
                            planBuilder.setStatus(PlanStatus.FAILED);
                        ...
                        if (actionsUpdated.getActionPlanInfo().hasBuyRi() &&
                            ...
                            if (PlanRpcServiceUtil.isScalingEnabled(planInstance.get().getScenario().getScenarioInfo())) {
                                ...
                                planAccess.updatePlanProgress(planId, oldInstance -> {
                            ...
                            planAccess.updatePlanProgress(planId, plan -> processActionsUpdated(plan, actionsUpdated));
                        }
    ...
    public void onProjectedTopologyAvailable(long projectedTopologyId, long topologyContextId) {
        ...
        if (topologyContextId != realtimeTopologyContextId) {
            try {
                ...
                final PlanInstance updatedPlan = planAccess.updatePlanProgress(topologyContextId, plan ->
    ...
    public void onProjectedTopologyFailure(long projectedTopologyId, long topologyContextId,
        ...
        if (topologyContextId != realtimeTopologyContextId) {
            try {
                final PlanInstance plan = planAccess.updatePlanProgress(topologyContextId, planBuilder -> {
                    planBuilder.setStatus(PlanStatus.FAILED);
    ...
    public void onSourceTopologyAvailable(long topologyId, long topologyContextId) {
        ...
        if (topologyContextId != realtimeTopologyContextId) {
            try {
                ...
                planAccess.updatePlanProgress(topologyContextId, plan ->
        ...
        if (planId != realtimeTopologyContextId) {
            ...
            try {
                if (statsAvailable.hasUpdateFailure()) {
                    ...
                    planAccess.updatePlanProgress(planId, plan -> {
                        plan.setStatus(PlanStatus.FAILED);
                        plan.setStatusMessage(statsAvailable.getUpdateFailure().getErrorMessage());
                    });
                } else {
                    planAccess.updatePlanProgress(planId, PlanProgressListener::processStatsAvailable);
                }
            } catch (IntegrityException e) {
            ...
            if (planId != realtimeTopologyContextId) {
                try {
                    if (statusUpdate.getType() == StatusUpdateType.PROJECTED_COST_UPDATE) {
                        ...
                        planAccess.updatePlanProgress(planId, p -> {
        ...
        if (topologyContextId != realtimeTopologyContextId) {
            try {
                logger.debug("Plan {} has a status message from market {}. Updating plan instance...",
                             topologyContextId, topologyId);
                    planAccess.updatePlanProgress(topologyContextId,
    ...
    /**
     * Utility class to make it more difficult to update a plan instance without checking for
     * terminal status.
     */
    private static class PlanAccess {
        private final PlanDao planDao;

        private PlanAccess(PlanDao planDao) {
            this.planDao = planDao;
        }

        public Optional<PlanInstance> getPlan(final long planId) {
            return planDao.getPlanInstance(planId);
        }

        public PlanInstance updatePlanProgress(final long planId, final Consumer<Builder> planBuilder)
                throws NoSuchObjectException, IntegrityException {
            return planDao.updatePlanInstance(planId, oldInstance -> {
                // If the plan is in a terminal state, we ignore further progress notifications.
                if (!PlanDTOUtil.isTerminalStatus(oldInstance.getStatus())) {
                    planBuilder.accept(oldInstance);
                } else {
                    logger.info("Plan {} (id: {}) in terminal state. Ignoring progress update,"
                                    + " and maintaining planStatus as {}", oldInstance.getName(), planId,
                            oldInstance.getStatus().name());
                }
            });
        }
    }
