...
public class PlanProgressListener implements ActionsListener, RepositoryListener, StatsListener {
            ...
            try {
                ...
                if (!planInstance.isPresent()) {
                    ...
                    if (actionsUpdated.getActionPlanInfo().hasBuyRi()) {
                        ...
                        if (resizeVMEnabled(planInstance.get())) {
    ...
    private boolean resizeVMEnabled(PlanInstance planInstance) {
        return planInstance.getScenario().getScenarioInfo().getChangesList()
                .stream()
                .filter(c -> c.hasSettingOverride())
                .map(c -> c.getSettingOverride())
                .filter(s -> s.getEntityType() == EntityType.VIRTUAL_MACHINE_VALUE)
                .map(s -> s.getSetting())
