...
/**
 * Validates the checksums for the DB migration files in this project's grafana DB endpoint against
 ...
 */
public class GrafanaDBMigrationScriptsTest extends TestDBMigrationChecksums {
    ...
    public void testMigrations() throws Exception {
        final File migrationDir = new File("src/main/resources/db/migration/grafana");
        Assert.assertTrue("Migration directory must exist and must be a directory.",
                          migrationDir.exists() && migrationDir.isDirectory());

        final File checksumFile = new File("src/test/resources/grafana-db-migration-checksums.properties");
