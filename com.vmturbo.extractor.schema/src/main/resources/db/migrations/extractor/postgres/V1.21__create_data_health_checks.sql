...
CREATE FUNCTION run_health_checks()
...
AS $$
  BEGIN
    ...
    RETURN NEXT;
    CALL hc_scope_oids_in_entity(passed, messages);
...
CREATE PROCEDURE hc_scope_oids_in_entity(INOUT passed boolean, INOUT messages text[])
AS $$
  DECLARE
    violations text[] = ARRAY[]::text[];
  BEGIN
