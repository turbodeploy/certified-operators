...
public class TierExcluder {
     ...
     */
    public void computeReasonSettings(List<ActionTO> m2Actions,
        ...
        m2Actions.forEach(m2Action -> {
            if (isTierExclusionAction(m2Action)) {
                tierExclusionActions.add(m2Action);
                Optional<Long> actionTarget = getActionTarget(m2Action);
                if (actionTarget.isPresent()) {
                    settingPoliciesToFetch.addAll(consumerOidToTierExclusionSettings.get(actionTarget.get()));
                }
            }
        });
        ...
        settingPolicyService.listSettingPolicies(ListSettingPoliciesRequest.newBuilder()
            ...
            .forEachRemaining(sp -> {
                Optional<List<Long>> excludedTiers = sp.getInfo().getSettingsList().stream()
                    ...
                    .findFirst()
                    .map(s -> s.getSortedSetOfOidSettingValue())
                    .map(s -> s.getOidsList());
        ...
        for (ActionTO tierExclusionAction : tierExclusionActions) {
            ...
            if (actionTarget.isPresent()) {
                ...
                if (primaryTier.isPresent()) {
                    ...
                    Set<Long> reasonSettings = candidateReasonSettings.stream()
                        .filter(setting -> {
     ...
     * So we clear that.
     */
    public void clearStateNeededForConvertToMarket() {
        familyToTiers.clear();
