...
public class EconomyCachesTest {
    ...
    public void testUpdateRealtimeCachedEconomy() {
        ...
        Map<Long, List<InitialPlacementDecision>> buyerPlacements = new HashMap() {{
            put(buyerOid, Arrays.asList(
                    new InitialPlacementDecision(buyerSlOid, Optional.of(pm1Oid), new ArrayList(), Optional.empty(), false,  Optional.of(EntityType.PHYSICAL_MACHINE_VALUE))));
        }};
    ...
    public void testUpdateHistoricalCachedEconomyNoBoundary() {
        ...
        Map<Long, List<InitialPlacementDecision>> buyerPlacements = new HashMap() {{
            put(buyerOid, Arrays.asList(
                    new InitialPlacementDecision(buyerSlOid, Optional.of(pm2Oid), new ArrayList(), Optional.empty(), false,  Optional.of(EntityType.PHYSICAL_MACHINE_VALUE))));
        }};
    ...
    public void testInitialPlacementUtilsExtractClusterBoundary() {
        ...
        placements.put(buyerOid, new ArrayList(Arrays.asList(
                new InitialPlacementDecision(slOid, Optional.of(st1Oid), new ArrayList<>(), Optional.empty(), false,  Optional.of(EntityType.STORAGE_VALUE)))));
    ...
    public void testUpdateTradersInHistoricalEconomyCache() {
        ...
        Economy clonedEconomy = InitialPlacementUtils.cloneEconomy(economy, false, new HashMap<>());
    ...
    public void testPopulateFailureInfos() {
        Basket basketSold = new Basket(Arrays.asList(new CommoditySpecification(SEGMENTATION_COMM_SPEC_TYPE),
                new CommoditySpecification(CLUSTER1_COMM_SPEC_TYPE)));
        Trader trader = new TraderWithSettings(1111, EntityType.PHYSICAL_MACHINE_VALUE, TraderState.ACTIVE, basketSold);
        ...
        List<FailureInfo> failureInfo = InitialPlacementUtils.populateFailureInfos(explanation, commTypeToSpecMap, true);
        Assert.assertEquals(0, failureInfo.get(0).getMaxQuantity(), 0.0001);
        // Verify the closest seller's cluster information is populated.
        Assert.assertTrue(failureInfo.get(0).getClosestSellerCluster().equals(cluster1Key));
        Assert.assertTrue(failureInfo.get(0).isFailedInRealtimeCache());
    }
    ...
    public void testAddTradersToHistoricalEconomyCache() {
        ...
        Economy clonedEconomy = InitialPlacementUtils.cloneEconomy(realeconomy, false, new HashMap<>());
