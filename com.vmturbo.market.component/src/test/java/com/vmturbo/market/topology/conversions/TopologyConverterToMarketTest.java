...
public class TopologyConverterToMarketTest {
    ...
    public void testTraderWithBicliqueCommodityConversion() throws Exception {
        CommodityConverter commodityConverter = Mockito.mock(CommodityConverter.class);
        TopologyConverter converter = Mockito.spy(
            new TopologyConverter(REALTIME_TOPOLOGY_INFO, false, 0.75f, marketPriceTable, commodityConverter));
        ...
        final CommodityDTOs.CommoditySoldTO economyDSPMSold = CommodityDTOs.CommoditySoldTO.newBuilder()
                        .setSpecification(CommoditySpecificationTO.newBuilder()
                            ...
                            .build())
                        .build();
        Mockito.doReturn(Optional.of(topologyCPUSold.getCommodityType())).when(commodityConverter)
                        .economyToTopologyCommodity(economyCPUSold.getSpecification());
        Mockito.doReturn(Optional.empty()).when(commodityConverter)
                .economyToTopologyCommodity(economyDSPMSold.getSpecification());
    ...
    public void testVMTraderToEntityConversion() throws Exception {
        CommodityConverter commConverter = Mockito.mock(CommodityConverter.class);
        TopologyConverter converter = Mockito.spy(
                new TopologyConverter(TopologyInfo.newBuilder()
                        .setTopologyType(TopologyType.PLAN)
                        .build(), false, 0.75f, marketPriceTable, commConverter));
        final TopologyDTO.CommoditySoldDTO topologyDSPMSold =
                TopologyDTO.CommoditySoldDTO.newBuilder()
                        .setCommodityType(CommodityType.newBuilder()
                                .setType(CommodityDTO.CommodityType.DSPM_ACCESS_VALUE)
        ...
        shoppingListInfos.setAccessible(true);
        shoppingListInfos.set(converter, shoppingListMap);
        CommoditySpecificationTO cpuCommSpec = CommoditySpecificationTO.newBuilder()
                .setBaseType(2)
                .setType(cpuType)
                .build();
        final CommodityDTOs.CommoditySoldTO economyCPUSold = CommodityDTOs.CommoditySoldTO.newBuilder()
                .setSpecification(cpuCommSpec)
                .build();
        final CommodityDTOs.CommoditySoldTO economyDSPMSold = CommodityDTOs.CommoditySoldTO.newBuilder()
                .setSpecification(CommoditySpecificationTO.newBuilder()
                        .setBaseType(base)
                        ...
                        .build())
                .build();
        Mockito.doReturn(Optional.of(topologyCPUSold.getCommodityType())).when(commConverter)
                .economyToTopologyCommodity(Mockito.eq(economyCPUSold.getSpecification()));
        Mockito.doReturn(Optional.of(topologyDSPMSold.getCommodityType())).when(commConverter)
                .economyToTopologyCommodity(Mockito.eq(economyDSPMSold.getSpecification()));
        Mockito.doReturn(cpuCommSpec).when(commConverter).commoditySpecification(cpuCommType);
        // create a topology entity DTO with DSPM sold
        TopologyDTO.TopologyEntityDTO expectedEntity = TopologyDTO.TopologyEntityDTO.newBuilder()
                .setEntityType(EntityType.VIRTUAL_MACHINE_VALUE)
                .setOid(10000L)
