 ...
 */
public class StorageAmountResolverTest {
    ...
    private final StorageAmountResolver storageAmountResolver = spy(new StorageAmountResolver(priceTableKeyStore, priceTableStore));
    ...
    public void testGetEndRange() {
        ...
        double adjustedAmount = storageAmountResolver.getEndRangeInPriceTier(33.0, 1L, 2L, STANDARD_HDD_DISK_TIER_OID);
        assertEquals(64, adjustedAmount, 0.001);
        adjustedAmount = storageAmountResolver.getEndRangeInPriceTier(4096.0, 1L, 2L, STANDARD_HDD_DISK_TIER_OID);
        assertEquals(4096, adjustedAmount, 0.001);
        // Return the input storage amount if there is no end-range. set.
        adjustedAmount = storageAmountResolver.getEndRangeInPriceTier(18.0, 1L, 2L, TEST_DISK_TIER_OID);
        assertEquals(18.0, adjustedAmount, 0.001);
        // If price table is not available, return the given storage amount.
        adjustedAmount = storageAmountResolver.getEndRangeInPriceTier(17.0, 1L, 2L, 1111L);
