...
public class SqlEntityEventsJournal implements EntityEventsJournal {
    ...
    public void getEventsBetween(long startTime, long endTime, Consumer<SavingsEvent> consumer) {
        readEvents(startTime, endTime, consumer);
    }
    ...
    public List<SavingsEvent> removeEventsBetween(long startTime, long endTime,
        ...
        final List<SavingsEvent> removedEvents = new ArrayList<>();
        getEventsBetween(startTime, endTime, entityOids, removedEvents::add);
     ...
     */
    private void readEvents(long startTime, long endTime, Consumer<SavingsEvent> consumer) {
        dsl.connection(conn -> {
            conn.setAutoCommit(false);
            try (Stream<SavingsEvent> savingsEventStream = DSL.using(conn, dsl.settings())
