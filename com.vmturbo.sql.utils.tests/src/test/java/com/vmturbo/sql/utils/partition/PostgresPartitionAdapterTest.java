...
/**
 * Test class for {@link PostgresPartitionAdapter} class.
 */
@RunWith(Parameterized.class)
public class PostgresPartitionAdapterTest extends MultiDbTestBase {
    ...
    public static Object[][] parameters() {
        return new Object[][]{
                // this test is specific to Postgres
                MultiDbTestBase.DBENDPOINT_POSTGRES_PARAMS
        };
    }
     ...
     */
    public PostgresPartitionAdapterTest(boolean configurableDbDialect, SQLDialect dialect)
    ...
    private PostgresPartitionAdapter adapter = null;
    ...
    @Before
    public void before() {
        this.adapter = new PostgresPartitionAdapter(dsl);
    ...
    public void testThatParitionCreationWorks() throws PartitionProcessingException {
        ...
        adapter.createPartition(
                mappedSchemaName, Tables.PART_TEST.getName(),
        ...
        // now do some checks of edge-case instants.
        assertThat(tableParts, not(coversInstant(Instant.ofEpochMilli(0L))));
        assertThat(tableParts, not(coversInstant(Instant.ofEpochMilli(5_000L))));
        ...
        assertThat(tableParts, not(coversInstant(Instant.ofEpochMilli(20_000L))));
        assertThat(tableParts, not(coversInstant(Instant.ofEpochMilli(29999L))));
    ...
    public void testThatPartitionDroppingWorks() throws PartitionProcessingException {
        ...
        assertThat(tableParts, not(coversInstant(Instant.ofEpochMilli(60_000L))));
        assertThat(tableParts, not(coversInstant(Instant.ofEpochMilli(69999L))));
        assertThat(tableParts, not(coversInstant(Instant.ofEpochMilli(70_000L))));
        assertThat(tableParts, not(coversInstant(Instant.ofEpochMilli(79999L))));
