 ...
 */
@Configuration
@Import({GroupComponentDBConfig.class, GroupDBEndpointConfig.class})
public class DbAccessConfig {

    @Autowired(required = false)
    private GroupComponentDBConfig groupComponentDBConfig;

    @Autowired(required = false)
    private GroupDBEndpointConfig groupDBEndpointConfig;
    ...
    public void startDbMonitor() {
        if (dbMonitorEnabled) {
            if (groupComponentDBConfig != null) {
                groupComponentDBConfig.startDbMonitor();
            } else {
                groupDBEndpointConfig.groupEndpoint().startDbMonitor();
            }
        }
    }
    ...
    public DSLContext dsl() throws SQLException, UnsupportedDialectException, InterruptedException {
        return FeatureFlags.POSTGRES_PRIMARY_DB.isEnabled()
                ? groupDBEndpointConfig.groupEndpoint().dslContext()
                : groupComponentDBConfig.dsl();
    }
    ...
    public DSLContext unpooledDsl()
        ...
        return FeatureFlags.POSTGRES_PRIMARY_DB.isEnabled()
                ? groupDBEndpointConfig.groupEndpoint().dslContext(false)
                : groupComponentDBConfig.unpooledDsl();
    }
    ...
    public DataSource dataSource() throws SQLException, UnsupportedDialectException, InterruptedException {
        return FeatureFlags.POSTGRES_PRIMARY_DB.isEnabled()
                ? groupDBEndpointConfig.groupEndpoint().datasource()
                : groupComponentDBConfig.dataSource();
    }
}