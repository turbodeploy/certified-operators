...
while [ ${COUNTER} -lt ${MAX_WAIT_ITERATIONS} ]; do
    ...
    LAST_FULL_GC_TIME=`python2.7 -c \
        "import urllib2; import socket; import sys; sys.tracebacklimit=0; socket.setdefaulttimeout($SOCKET_TIMEOUT_SECONDS); print(urllib2.urlopen('http://localhost:8080/health').read())" | \
        grep -oP 'lastFullGC \d+' | \
        awk '{print $NF}'`

    # if the python script to check the endpoint finished successfully, check the GC time it found.
    # $LAST_FULL_GC_TIME will be an empty string if the script failed, in which case we should
    # go right into the next wait cycle.
    if [ ! -z "$LAST_FULL_GC_TIME" ]; then
        # if $LAST_FULL_GC_TIME was reported, then check to see if the time was after our dump
        # request time. If it is, we know the histogram has finished.
...
if [ ${COUNTER} -ge ${MAX_WAIT_ITERATIONS} ]; then
    echo "Histogram dump took too long. Abandoning after $(($COUNTER*($SLEEP_TIME_SECONDS + $SOCKET_TIMEOUT_SECONDS))) seconds."
else
