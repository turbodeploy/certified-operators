...
# utility methods to work with mysql database

def get_db_config_from_file():
    """
    Read the db.ini file as a primary source for db properties.
    The DB_PATHS_CONFIG environment variable is used to locate the config file..

    :return: parsed config file
    """
    config = ConfigParser(interpolation=None)
    config.read(os.environ['DB_PATHS_CONFIG'])
    return config
...
def get_db_config(property_name, db_type, endpoint_name, components=None):
    ...
    should not be used for properties that are available in the db.ini file, since the latter
    will be used directly by the db server when accessing its internal data.
    ...
    """
    value = None
    db_type_aliases = {'mysql': ['mysql', 'mariadb']}.get(db_type)
...
def get_connection(db_type, host, port, database, user, password):
    ...
    :return: connection
    """
    connect_func = {'mysql': mysql.connector.connect}.get(db_type)
    if connect_func:
...
def get_endpoint_connection(db_type, endpoint_name, components=None, host=None, port=None,
    ...
    # explicitly compare to None since '' is a correct value sent in for a root mysql connection
...
def get_db_default(property_name, db_type, components=None):
    ...
    may be overridden by db.ini or CR values, but values returned by this method will be used
    otherwise.
...
def get_db_dsn():
    ...
    dsn = db_type + '://' + config('user') + ':' \
          + get_from_auth(db_type, 'rootPassword', config('component')) \
...
def get_db_creds():
    ...
    config = lambda key : os.environ.get('DATABASE_' + key.upper(), database.get(key))
    db_type = config('type')
    creds = config('user') + ':' + get_from_auth(db_type, 'rootPassword', config('component'))
...
if __name__ == '__main__':
    db_config = get_db_config_from_file()
    main()