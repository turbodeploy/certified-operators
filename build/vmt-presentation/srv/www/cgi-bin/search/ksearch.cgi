...
sub returnresults {	# creates HTML page from template file
	...
	if ($add_plus) {
		$options .= '&all=1';
		$h{add_plus} = '<input type="checkbox" name="all" value="1" checked="checked" />+';
	} else {
	...
	if ($case_sensitive) {
		...
		$h{c} = '<input type="checkbox" name="c" value="s" checked="checked" />Case Sensitive';
	} else {
		$casesearch = "";
        ...
        if ($show_matches && $SHOW_MATCHES) {
                ...
                $h{show_matches} = '<input type="checkbox" name="showm" value="'.$show_matches.'" checked="checked" />Show Matches in Descriptions';
        } elsif ($SHOW_MATCHES) {
	...
	if ($whole_word) {
		$options .= '&amp;w=1';
		$h{w} = '<input type="checkbox" name="w" value="1" checked="checked" />Whole Words Only';
	} else {
		$h{w} = '<input type="checkbox" name="w" value="1" />Whole Words Only';
	}
	if ($all) {
		$options .= '&amp;st=1';
		$h{st} = '<input type="checkbox" name="st" value="1" checked="checked" />Include Stop-Terms';
	} else {
	...
	if ($search_body) {
		$search_sources .= " Body,";
		$options .= '&amp;b=1';
		$h{b} = '<input type="checkbox" name="b" value="1" checked="checked" />Body';
	} else {
	...
	if ($search_title) {
		...
		$options .= '&amp;t=1';
		$h{t} = '<input type="checkbox" name="t" value="1" checked="checked" />Title';
	} else {
	...
	if ($search_meta_description) {
		...
		$options .= '&amp;d=1';
		$h{d} = '<input type="checkbox" name="d" value="1" checked="checked" />Meta-Description';
	} else {
	...
	if ($search_meta_keyword) {
		...
		$options .= '&amp;k=1';
		$h{k} = '<input type="checkbox" name="k" value="1" checked="checked" />Meta-Keywords';
	} else {
	...
	if ($search_meta_author) {
		...
		$options .= '&amp;au=1';
		$h{au} = '<input type="checkbox" name="au" value="1" checked="checked" />Meta-Authors';
	} else {
	...
	if ($search_alt_text) {
		...
		$options .= '&amp;alt=1';
		$h{alt} = '<input type="checkbox" name="alt" value="1" checked="checked" />Alt-Text';
	} else {
	...
	if ($search_links) {
		...
		$h{l} = '<input type="checkbox" name="l" value="1" checked="checked" />Links';
	} else {
	...
	if ($search_url) {
		$search_sources .= " Url,";
		$options .= '&amp;u=1';
		$h{u} = '<input type="checkbox" name="u" value="1" checked="checked" />Url';
	} else {
		$h{u} = '<input type="checkbox" name="u" value="1" />Url' if $URL;
	}
	$search_sources =~ s/,$//;
	if ($SORT_BY eq "Matches") {
		$h{sort} = '<option selected="selected">Matches</option> <option>Scores</option> <option>Dates</option> <option>Sizes</option> <option>Titles</option> <option>File Names</option>';
	} elsif ($SORT_BY eq "Scores") {
		$h{sort} = '<option>Matches</option> <option selected="selected">Scores</option> <option>Dates</option> <option>Sizes</option> <option>Titles</option> <option>File Names</option>';
	} elsif ($SORT_BY eq "Dates") {
		$h{sort} = '<option>Matches</option> <option>Scores</option> <option selected="selected">Dates</option> <option>Sizes</option> <option>Titles</option> <option>File Names</option>';
	} elsif ($SORT_BY eq "Sizes") {
		$h{sort} = '<option>Matches</option> <option>Scores</option> <option>Dates</option> <option selected="selected">Sizes</option> <option>Titles</option> <option>File Names</option>';
	} elsif ($SORT_BY eq "Titles") {
		$h{sort} = '<option>Matches</option> <option>Scores</option> <option>Dates</option> <option>Sizes</option> <option selected="selected">Titles</option> <option>File Names</option>';
	} else {
		$h{sort} = '<option>Matches</option> <option>Scores</option> <option>Dates</option> <option>Sizes</option> <option>Titles</option> <option selected="selected">File Names</option>';
	}
	$sortby = '&amp;sort='.$SORT_BY;
	if ($RESULTS_PER_PAGE == 5) {
		$h{display} = '<option selected="selected">5</option> <option>10</option> <option>25</option> <option>50</option> <option>100</option>';
	} elsif ($RESULTS_PER_PAGE == 10) {
		$h{display} = '<option>5</option> <option selected="selected">10</option> <option>25</option> <option>50</option> <option>100</option>';
	} elsif ($RESULTS_PER_PAGE == 25) {
		$h{display} = '<option>5</option> <option>10</option> <option selected="selected">25</option> <option>50</option> <option>100</option>';
	} elsif ($RESULTS_PER_PAGE == 50) {
		$h{display} = '<option>5</option> <option>10</option> <option>25</option> <option selected="selected">50</option> <option>100</option>';
	} elsif ($RESULTS_PER_PAGE == 100) {
		$h{display} = '<option>5</option> <option>10</option> <option>25</option> <option>50</option> <option selected="selected">100</option>';
	} else {
		$h{display} = '<option selected="selected">'.$RESULTS_PER_PAGE.'<option>5</option> <option>10</option> <option>25</option> <option>50</option> <option>100</option>';
	}
	...
	if ($query->param('help') == 1) {
		...
		$h{header} = 'Enter search terms';
	} elsif (@none) {		
		$h{header} = 'No matches for <i>'.$bare_query.'</i>:';
	} elsif (!$results) {
		$h{header} = 'No matches for <i>'.$bare_query.'</i>.<br><br>Either the documents don\'t contain this term, or the term is excluded from search.';
	} else {
		$h{help} = $RESULTS_TITLE if $RESULTS_TITLE;
		my $s = ($totalmatches == 1 ? "" : "es");		
		$h{header} = $totalmatches.' total matches for <i>'.$bare_query.'</i> in the following topics:';
	}
	...
	foreach ((@sortedanswers)[$first..$last]) {
		my ($title, $desc);
		$desc = "";
		### titles_db{$_} is in the form "<bookName>: <topicTitle>"
		### Using split to get the second token.  Extra leading space is ignored by browser.
		my @toks = split(/:/, $titles_db{$_});
		$title = $toks[1];
	...
	if ($currentpage == 1) {
		...
		if ($lastpage > $currentpage) {
			$h{next} = " &nbsp; <a href=\"$SEARCH_URL?".$FORM_INPUT_NAME."=".$query_str."&amp;display=".$RESULTS_PER_PAGE."&amp;p=2".$subsearch_string.$options.$sortby."\"> $NEXT</a>\n";
		} else {
			$h{next} = "";
		}
	} elsif ($currentpage == $lastpage) {
		$h{previous} = "<a href=\"$SEARCH_URL?".$FORM_INPUT_NAME."=".$query_str."&amp;display=".$RESULTS_PER_PAGE."&amp;p=".($lastpage-1).$subsearch_string.$options.$sortby."\">$PREVIOUS </a> &nbsp;\n";
		$h{next} = "";
	} else {
		$h{previous} = "<a href=\"$SEARCH_URL?".$FORM_INPUT_NAME."=".$query_str."&amp;display=".$RESULTS_PER_PAGE."&amp;p=".($currentpage-1).$subsearch_string.$options.$sortby."\">$PREVIOUS </a> &nbsp; \n";
		$h{next} = " &nbsp; <a href=\"$SEARCH_URL?".$FORM_INPUT_NAME."=".$query_str."&amp;display=".$RESULTS_PER_PAGE."&amp;p=".($currentpage+1).$subsearch_string.$options.$sortby."\"> $NEXT</a>\n";
	}
	if ($lastpage == 1) {
		$h{navbar} = "";
	} else {
		for (1..$lastpage) {
			...
			if ($_ != $currentpage) {
				$h{navbar} .= " <a href=\"$SEARCH_URL?".$FORM_INPUT_NAME."=".$query_str."&amp;display=".$RESULTS_PER_PAGE."&amp;p=".$_.$subsearch_string.$options.$sortby."\">$resultspage</a> \n";
			} else {
	...
	if ($query->param('help') == 1) {	# add 'back' link to help page
		$h{tips} = '<a href="'.$SEARCH_URL.'?'.$FORM_INPUT_NAME.'='.$query_str.'&amp;p='.$currentpage.'&amp;display='.$RESULTS_PER_PAGE.$subsearch_string.$options.$sortby.'"><strong>Back</strong></a>';
		$h{navbar} = ""; $h{next} = ""; $h{previous} = "";
		$currentpage = 1;
	} else {				# add 'tips' link to results page
		$h{tips} = '<a href="'.$SEARCH_URL.'?'.$FORM_INPUT_NAME.'='.$query_str.'&amp;p='.$currentpage.'&amp;help=1&amp;display='.$RESULTS_PER_PAGE.$subsearch_string.$options.$sortby.'"><strong>Tips</strong></a>';
	}
...
#	if (timestr(timediff(new Benchmark, $t0)) =~ /^(.+)$/i && !@none && $results) { # wallclock, usr, and sys times
		$h{searchtime} = '<br />Required Time: <strong>'.($1 == 1 ? "$1 second" : "$1 seconds")."</strong>";
