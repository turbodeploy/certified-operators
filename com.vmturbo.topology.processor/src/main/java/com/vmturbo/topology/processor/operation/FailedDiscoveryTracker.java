...
public class FailedDiscoveryTracker implements OperationListener, TargetStoreListener {
    ...
    private static Map<Long, DiscoveryFailure> targetToFailedDiscoveries = new HashMap<>();
     ...
     */
    @Nonnull
    public static Map<Long, DiscoveryFailure> getFailedDiscoveries() {
        ...
        public DiscoveryFailure(Discovery discovery) {
            ...
            if (!errorTypes.isEmpty())  {
                ...
                logger.warn("Have had a discovery failure for target with id {} "
                                + "but the error type is not set", discovery.getTargetId());
            }
