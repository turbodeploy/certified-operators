 ...
 */
public class V_01_01_16__GCP_Remove_Billing_Probe_Targets extends AbstractProbeTargetMigration {

    private final Logger logger = LogManager.getLogger();

    private static final String MIGRATION_PURPOSE =
        "Migration to remove GCP Billing probes and targets to allow negotiated pricing feature";
     ...
     */
    public V_01_01_16__GCP_Remove_Billing_Probe_Targets(@Nonnull KeyValueStore keyValueStore) {
        super(keyValueStore);
    }
    ...
    boolean shouldUpdateProbe(String probeId, String probeType, JsonObject probeJson) {
        return probeType.equalsIgnoreCase(SDKProbeType.GCP_BILLING.getProbeType());
    }

    @Override
    void updateProbe(String probeId, String probeType, JsonObject probeJson, String consulProbeKey) {

        logger.info("Removing GCP Billing probe key '{}' from the KV store", consulProbeKey);
    ...
    void processIndividualTarget(JsonObject secretFields, JsonObject targetInfoObject, String consulTargetKey) {
        ...
        if (probeType.equalsIgnoreCase(SDKProbeType.GCP_BILLING.getProbeType())) {
