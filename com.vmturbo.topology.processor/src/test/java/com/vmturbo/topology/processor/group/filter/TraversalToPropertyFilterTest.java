...
public class TraversalToPropertyFilterTest {
    ...
    public void testSupplyChainDeeperThanMaxRecursionDepth() {
        ...
        assertThat(filterOids(filter, topologyGraph, 0L), is(empty()));
    }
    ...
    private static PropertyFilter entityTypeFilter(@Nonnull final EntityType entityType) {
        ...
        doAnswer(invocation -> ((Vertex)invocation.getArguments()[0]).getEntityType() == entityType.getNumber())
            .when(entityTypeFilter).test(any(Vertex.class));
