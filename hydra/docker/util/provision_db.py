...
def provision_mysql(host, database, user, password, endpoint_name, component):
    ...
    if can_connect('mysql', host, port, database, user, password):
        ...
        return
    # obtain root credentials if we need to do provisionng
    try:
...
def main():
    ...
    try:
        if config('type') == 'mysql':
            provision_mysql(config('host'), config('name'), config('user'), db.get_from_auth('mysql', 'rootPassword', ['component']), config('endpoint_name'), config('component'))
        else:
