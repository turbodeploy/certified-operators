...
public class ActionDTOUtilTest {
    ...
    /**
     * Verify the list of involved entities for Move Volume Action in MCP.
     *
     * @throws UnsupportedActionException should not be thrown.
     */
    @Test
    public void testInvolvedEntitiesMoveVolumeAction() throws UnsupportedActionException {

        final long onPermStorageId = 1002L;

        final long cloudVmId =  2010L;
        final long cloudVvId = 2060L;
        final long cloudRegionId = 2054L;
        final long cloudStorageTierId = 2057L;

        // Move Volume
        final Action moveVolumeAction = Action.newBuilder()
            .setId(1L)
            .setDeprecatedImportance(1.0)
            .setInfo(ActionInfo.newBuilder()
                .setMove(Move.newBuilder()
            ...
            .setExplanation(Explanation.newBuilder()
                .setMove(MoveExplanation.newBuilder()
                    .addChangeProviderExplanation(ChangeProviderExplanation.getDefaultInstance())))
            .build();

        List<ActionEntity> involvedEntities = ActionDTOUtil.getInvolvedEntities(
            moveVolumeAction, InvolvedEntityCalculation.INCLUDE_ALL_STANDARD_INVOLVED_ENTITIES);
        Set<Long> actionEntityIds = involvedEntities.stream().map(ActionEntity::getId).collect(Collectors.toSet());
        // Result should NOT include cloud VM
        assertEquals(Sets.newHashSet(onPermStorageId, cloudVvId, cloudRegionId, cloudStorageTierId), actionEntityIds);
    }
