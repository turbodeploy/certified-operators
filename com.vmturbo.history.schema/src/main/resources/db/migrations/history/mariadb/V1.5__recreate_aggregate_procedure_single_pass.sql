/* V1.5_recreate_aggregate_single_pass.sql */
...
CREATE DEFINER=CURRENT_USER PROCEDURE `aggregate`(IN statspref CHAR(10))
  aggregate_proc:BEGIN
    ...
    set @sql=concat('select min(snapshot_time) into @min_created_time from ',statspref,'_stats_latest');
    PREPARE stmt from @sql;
    EXECUTE stmt;
    DEALLOCATE PREPARE stmt;

    set @sql=concat('select max(snapshot_time) into @max_created_time from ',statspref,'_stats_latest');
    PREPARE stmt from @sql;
    EXECUTE stmt;
    DEALLOCATE PREPARE stmt;
    ...
    IF (@aggregation_meta_exists=0) THEN
        ...
        set @sql=concat('SELECT min(snapshot_time) INTO @last_aggregated_time FROM ',statspref,'_stats_latest');
        PREPARE stmt from @sql;
        EXECUTE stmt;
        DEALLOCATE PREPARE stmt;
    END IF;

    /* FUTURE EXIT IF NOTHING TO AGGREGATE
    ...
    */
    ...
    /* Aggregation has never been performed.  Set the starting point to now() */
    if (last_aggregated_by_hour_time='0000-00-00 00:00:00' OR last_aggregated_by_hour_time IS NULL) THEN
        SET last_aggregated_by_hour_time = now();
    end if;
    ...
    if (last_aggregated_by_day_time='0000-00-00 00:00:00' OR last_aggregated_by_day_time IS NULL) THEN
        SET last_aggregated_by_day_time = now();
    end if;
    ...
    set @sql=concat('create or replace view ',statspref,'_daily_ins_vw as
        ...
        from ',statspref,'_stats_by_hour a, aggregation_meta_data b where a.snapshot_time > b.last_aggregated_by_day and b.aggregate_table=\'',statspref,'_stats_latest\'  group by day_key');
...
',statspref,'_stats_by_day.aggregated=1');
    ...
    if (last_aggregated_by_month_time='0000-00-00 00:00:00' OR last_aggregated_by_month_time IS NULL) THEN
       SET last_aggregated_by_month_time = now();
     end if;
     ...
     set @sql=concat('create or replace view ',statspref,'_monthly_ins_vw as
        ...
        sum(samples) as samples,
        sum(new_samples) as new_samples
        from ',statspref,'_stats_by_day a, aggregation_meta_data b where a.snapshot_time > b.last_aggregated_by_month and b.aggregate_table=\'',statspref,'_stats_latest\'  group by month_key');
