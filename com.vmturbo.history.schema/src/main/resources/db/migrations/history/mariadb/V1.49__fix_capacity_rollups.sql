/*
 * Here we fix a bug in rollups that has existed from the very first migration in history component,
 * namely that capacity (and later also effective_capacity) are not rolled up to max values in
 * rollup tables.
 */
