/*
  Modify the "rotate_partition" procedure to fix the partition creation for
  monthly tables. Instead of creating partitions for the future months, we
  were just creating partitions for the next few days which caused most of
  the currently monthly data to be in the future table. Since for safety,
  we truncate future partition after every repartition, the monthly table
  ended up being always empty.

  Note: The procedure is the same as before, The change is only on line 90
  to fix the calculation of the future partition date for the monthly table.
*/
