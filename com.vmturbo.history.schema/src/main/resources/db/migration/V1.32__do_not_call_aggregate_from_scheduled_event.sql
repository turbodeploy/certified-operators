-- NOTE: This migration is a copy of the migration V1.17, repeated here so that it will be
...
-- The lack of coordination of rollup processing with topology ingestion in history component is
-- causing very visible anomalies, because the aggregate() stored proc may see only some of the
-- entities for a given entity type, if it runs while ingestion is happening.
--
-- Here we remove calls to aggregate and market_aggregate procs from the aggregate_stats event
-- body. And since that means the event doesn't cause any aggregation - only repartitioning - we
-- rename the event accordingly.
--
-- In addition, since we will be running aggregations for individual tables in parallel, the
-- existing CHKAGGR function used by the entities aggregation procedure is no longer appropriate.
-- The check should no longer be needed, becuase invocation will be controlled by history
-- component.
--
-- Besides the above issues, we're fixing an issue discovered during testing, involving the
-- improper use of the last_aggregated field in the aggregation_meta_data table, which only
-- ever set by a feature of MariaDB that, by default, sets timestamp fields to current time
-- when they would otherwise be set to null. Also removed some obsolete code that was left
-- over from a prior version.
--
-- The V1.15 migration contains the definition of aggregate stored proc that formed the basis
-- of these changes.

DELIMITER //
DROP EVENT IF EXISTS `aggregate_stats_event`//
DROP EVENT IF EXISTS `repartition_stats_event`//
CREATE EVENT `repartition_stats_event`
    ON SCHEDULE EVERY 10 MINUTE STARTS '2018-03-09 17:32:36'
