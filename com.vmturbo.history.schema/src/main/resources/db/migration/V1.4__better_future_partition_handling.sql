/*
  Modify the "rotate_partition" procedure in order to better handle data that might pile up.

  As a safety measure in order to not pile up data, and fill up the disk (which will result in
  stopping the whole platform), we want to make sure that the future partition is empty,
  so that when we will trigger the repartition to create new ones, the future will not contain any data,
  and hence we will not copy any data over to the new partitions (which will take additional time).

  This means that if for some reason (we are slow, and cannot keep up with incoming data/aggregation)
  the future partition contains some data, those data will be dropped.
  This choice has been done to maintain the platform alive, even if that means to drop data, instead
  of keep all the data, but then die because of disk full.
