...
CREATE DEFINER=CURRENT_USER PROCEDURE `aggregate`(IN statspref CHAR(10))
  aggregate_proc:BEGIN
    ...
    set @sql=concat('SELECT 1 FROM ',statspref,'_stats_latest where snapshot_time>(SELECT last_aggregated FROM aggregation_meta_data WHERE aggregate_table=\'',statspref,'_stats_latest\') limit 1');
    select @sql;
    PREPARE stmt from @sql;
    EXECUTE stmt;
